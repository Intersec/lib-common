...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--output-pystub',
        help=('The output file where to write the python stub file'),
        type=pathlib.Path,
        required=True,
    )
...
def dump_channel_types(
    ...
    dump_channel_type(
        plugin, 'AsyncChannel', 'AsyncModule', module_names, output_file
    )
    dump_channel_type(
        plugin, 'ChannelServer', 'ModuleServer', module_names, output_file
    )
...
def process_dso(dso_path: pathlib.Path, output_pystub: pathlib.Path) -> None:
    ...
    packages = sorted(
        pkg_name
        for pkg_name, pkg_type in plugin.__dict__.items()
