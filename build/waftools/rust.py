...
class CargoBuildBase(Task.Task):  # type: ignore[misc]
    ...
    def run_cargo(self) -> None:
        # Note: `cargo build` is run even for packages that do not produce any
        # output handled by waf (pure Rust libraries). Building them here is
        # not needed for the link, but it is needed for the ordering of the
        # bindings generation: a package blocks the items already generated by
        # its dependencies (see `waf-cargo-build`), so the dependencies must
        # have generated their `bindings_items.json` beforehand. Cargo does not
        # order the build scripts of a package and of its dependencies, only
        # waf does, through the task dependencies. This costs nothing: the
        # produced rlib is reused by the packages depending on it.
