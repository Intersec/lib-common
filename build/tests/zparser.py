...
class StreamParser:
    ...
    def parse_line(self, stream_line: bytes | str) -> None:
        ...
        for line in lines:
            ...
            # ----------------
            # Example:
            # :Core was generated by `/home/buildbot/bb-rocky9-...'.
            ...
            # ---------------------
            # Examples:
            # starting suite one/doc/zchk-lpm-api.py...
            # starting suite one/zchk...
            # starting suite one/behave...
            # starting suite one/www/tests/testem/mcms/testem.json...
            ...
            # Start of a group test in test suite
            # -----------------------------------
            # Examples:
            # 1..1 Seamless refresh of subscribed...
            # 1..13 Some basic checks on One/Igloo compatibility
            ...
            # End of a test suite
            # -------------------
            # There are two kinds of log to describe the end of a test suite
            #
            # Error kind examples:
            # TEST SUITE one/behave FAILED (1254 seconds)
            # TEST SUITE .../zchk-bigdata-dc.py FAILED (364 seconds)
            #
            # Success kind examples:
            # done (431 seconds)
            ...
            # End of a scenario
            # -----------------
            # Format: current test nb | sts | name | duration | nb of steps
            # in the case of zchk, there is no step count
            #
            # Examples:
            # 1 skip Failover of active minion   # (0.000s) 0 steps
            # 1 pass Global announcements   # (0.088s) 19 steps
            # 8 pass Check task RPCs are hidden ...  # (0.041s) 6 steps
            # 1 fail test_upload_workflow # (33.001s)
            ...
            # Match screenshot url
            r = RE_SCREEN.match(line)
            if r is not None:
                self.screenshot = r.group('url')

            # Matching step in scenario
            # scenario number - step number | status | step name |
            #   feature name | line number | step duration
            # #  1-1  pass <given One monohost is running> announcement.feature:19    # (0.003s)
            # #  8-2  pass <given workspace role "not_api_users" exists> base.feature:137   # (0.003s)
            ...
            # Analyse report lines that start with a colon (':')
            # --------------------------------------------------
            # These lines are added to the end of the log output during
            # the first pass done by `run_check.sh` to generate a status and
            # resume errors.
            #
            # In the second execution of `zparser`, triggered by `buildbot`,
            # we re-parse the log specifically to extract detailed information
            # needed for generating the final HTML report. This includes:
            # - Python backtraces for failed steps
            # - Browser logs
            # - Core process-related messages
            # - Step-level test actions (Given/When/Then)
            #
            # Example lines:
            # :   <when "stop service core">
            # :   Traceback:
            # :   pypf.steps.generic.SubStepFailed: ...
            if line.startswith(':'):
                if self.error:
