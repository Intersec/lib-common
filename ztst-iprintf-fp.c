/*
 * Test program for floating point formatting from Cygnus C library
 *
...
sprint_double_type sprint_doubles[] = {
    ...
    X(__LINE__, -1.879343051002554e-20, "   -0.", "%#6.f" )
    ...
    X( __LINE__, -2.190642494146884e-27, " -0.", "%#4.f" )
    ...
    X(__LINE__, -4.087336043219081e-10, "-0.", "%+#0.f" )
    ...
    X(__LINE__, -4.273844276097008e-09, " -0.", "%#4.f" )
    ...
    X(__LINE__, -6.492942931343439e-08, "-0.", "%+#2.0f" )
    ...
    X(__LINE__, -9.695661526907136e-24, "   -0.", "%#6.f" )
    ...
    X(__LINE__, 1.130331982972407e-06,  "+0.", "%+#1.f" )
    ...
    X(__LINE__, 1.208091664701942e-13,  " 0.", "%#3.f" )
    ...
    X(__LINE__, 1.753871003884151e-26,  "0.", "%#.0f" )
    ...
    X(__LINE__, 2.639704195859799e-11,  "0.", "%#.0f" )
    ...
    X(__LINE__, 4.756952432926979e-29,  " 0.", "%#3.f" )
    ...
    X(__LINE__, 5.578254777281501e-21,  "0.", "%#.0f" )
    ...
    X(__LINE__, 7.315587463572568e-04,  "0.", "%#.0f" )
    ...
    X(__LINE__, 7.593317194045158e-12,  "+0.", "%+#1.f" )
    ...
    X(__LINE__, 9.227419774250317e-28,  " 0.", "%#3.f" )
...
//    X(__LINE__, 16,                     "0x1.0p+4", "%.1a" )
//    X(__LINE__, 16,                     "0x1.00000000000000000000p+4", "%.20a" )
//    X(__LINE__, 4444.88888888,          "4445", "%2.F" )
...
#endif
{
    ...
    for (;; result++, desired++) {
        ...
        if (digits_seen >= required_precision
        &&  *result >= '0' && *result <= '9'
        &&  *desired >= '0' && *desired <= '9')
            continue;
        else
...
#define BSIZE 1024
    ...
    for (dptr = sprint_doubles; dptr->line; dptr++) {
        ...
        if (!matches(buffer, dptr->result)) {
            errcount++;
            printf("%s:%d: printf(\"%s\", %g) -> "
                   "\"%s\", expected \"%s\".\n",
                   __FILE__, dptr->line,
                   dptr->format_string, dptr->value,
                   buffer, dptr->result);
        }
        ...
        if (dptr->value != d) {
            errcount++;
            printf("%s:%d: error: string is \"%s\", value is %g.\n",
                   __FILE__, dptr->line, buffer, d);
        }
        testcount++;
    }
