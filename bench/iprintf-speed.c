...
static long snprintf_event(const mcms_event_t *event)
{
    ...
    return snprintf(
        buf, sizeof(buf), "%d|%c|%lld|%d|%d|%u|%d|\n", event->stamp,
        event->type, (long long)event->msisdn, event->camp_lineno,
        event->camp_id, event->remote_id, event->payload_len
    );
}

ZBENCH_GROUP_EXPORT(iprintf_bench){ZBENCH(snprintf){int i = 0;
