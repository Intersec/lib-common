...
This requires the `bindings_items.json` of a package to be generated before the
one of the packages depending on it. Cargo gives no such guarantee (it runs the
build scripts of a package and of its dependencies in parallel), so this
ordering is enforced by waf, through the dependencies declared with `use`
between the Rust targets. Hence a Rust package using the bindings of another
one *must* declare it in the `use` of its waf target, otherwise its bindings
will be generated with an incomplete block list and the same C types will be
defined twice.
...
`proc-macro`), waf produces no output, but still calls `cargo build` on these
packages so that their bindings are generated in the order of the waf
dependencies (see above). The resulting rlib is reused by the packages
depending on it, so this costs nothing.
