...
#define PROTECT_ERRNO(expr)                                                  \
    ({                                                                       \
        int save_errno__ = errno;                                            \
        expr;                                                                \
        errno = save_errno__;                                                \
    })

#define ERR_RW_RETRIABLE(err) ((err) == EINTR || (err) == EAGAIN)
#define ERR_CONNECT_RETRIABLE(err) ((err) == EINTR || (err) == EINPROGRESS)
...
void ps_panic_sighandler(
    int signum, siginfo_t *nullable si, void *nullable addr
);
...
off_t filecopyat(
    int dfd_src, const char *nonnull name_src, int dfd_dst,
    const char *nonnull name_dst
);
...
#ifdef __has_blocks
typedef int(BLOCK_CARET on_file_b)(
    const char *nonnull dir, const linux_dirent_t *nonnull de
);
 ...
 */
int list_dir(
    const char *nonnull path, unsigned flags, on_file_b nullable on_file
);
#endif
...
__must_check__ ssize_t xwrite_file_extended(
    const char *nonnull path, const void *nonnull data, ssize_t dlen,
    int flags, mode_t mode
);

static inline __must_check__ ssize_t
xwrite_file(const char *nonnull path, const void *nonnull data, ssize_t dlen)
{
    return xwrite_file_extended(
        path, data, dlen, O_WRONLY | O_CREAT | O_TRUNC, 0644
    );
}

static inline __must_check__ ssize_t xappend_to_file(
    const char *nonnull path, const void *nonnull data, ssize_t dlen
)
{
    return xwrite_file_extended(
        path, data, dlen, O_WRONLY | O_CREAT | O_APPEND, 0644
    );
}

__must_check__ ssize_t xwrite(int fd, const void *nonnull data, ssize_t dlen);
__must_check__ ssize_t xwritev(int fd, struct iovec *nonnull iov, int iovcnt);
__must_check__ ssize_t
xpwrite(int fd, const void *nonnull data, ssize_t dlen, off_t offset);
__must_check__ int xftruncate(int fd, off_t offs);
__must_check__ int xread(int fd, void *nonnull data, ssize_t dlen);
__must_check__ int
xpread(int fd, void *nonnull data, ssize_t dlen, off_t offset);
...
__attr_nonnull__((1)) static inline int p_fclose(
    FILE * nullable * nonnull fpp
)
{
    FILE *fp = *fpp;
...
__attr_nonnull__((1)) static inline int p_closedir(
    DIR * nullable * nonnull dirp
)
{
    DIR *dir = *dirp;
...
__attr_nonnull__((1)) static inline int p_close(int *nonnull hdp)
{
    int hd = *hdp;
    *hdp = -1;
    if (hd < 0) {
