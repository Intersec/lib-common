...
#undef E
    ...
    if (strequal(ext, ".gz")) {
        ...
        *(char *)ext = '\0';
        ext = path_extnul(fpart);
    } else if (strequal(ext, ".Z")) {
        ob_adds(ob, "Content-Encoding: compress\r\n");
        has_enc = true;
        *(char *)ext = '\0';
        ext = path_extnul(fpart);
    } else if (strequal(ext, ".bz2")) {
...
static void httpd_query_reply_make_index_(
    httpd_query_t *q, int dfd, const struct stat *st, bool head
)
{
    ...
    if (!head) {
        ...
        while ((de = readdir(dir))) {
            ...
            if (S_ISDIR(tmp.st_mode)) {
                ob_addf(
                    ob, "<a href=\"%s/\">%s/</a><br>", de->d_name, de->d_name
                );
...
static void httpd_trigger__dir_cb(
    httpd_trigger_t *cb, httpd_query_t *q, const httpd_qinfo_t *req
)
{
    t_scope;
    ...
    memcpyz(
        mempcpy(buf, cb2->dirpath, cb2->dirlen), req->query.s,
        ps_len(&req->query)
    );
