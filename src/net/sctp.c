...
ssize_t sctp_sendv(
    int sd, const struct iovec *iov, int iovlen,
...
int sctp_send(
    int s, const void *msg, size_t len, const struct sctp_sndrcvinfo *sinfo,
    int flags
)
{
...
int sctp_sendmsg(
    int sd, const void *_msg, size_t len, struct sockaddr *to,
...
int sctp_recvmsg(
    int sd, void *_msg, size_t len, struct sockaddr *from, socklen_t *fromlen,
...
int sctp_peeloff(int fd, sctp_assoc_t associd)
{
    ...
    if ((err = getsockopt(
             fd, SOL_SCTP, SCTP_SOCKOPT_PEELOFF, &peeloff, &peeloff_size
         )) < 0)
    {
        return err;
    }
...
int sctp_connectx_ng(
    int fd, struct sockaddr *addrs, int count, sctp_assoc_t *nullable id
)
{
    int res;
    int size = sctp_addr_len((const sockunion_t *)addrs, count);
    ...
    res =
        getsockopt(fd, SOL_SCTP, SCTP_SOCKOPT_CONNECTX3, &opt_val, &opt_len);
...
int sctp_getaddrs(
    int fd, int optnum, sctp_assoc_t id, struct sockaddr *addrs, int addr_size
)
{
...
void sctp_dump_notif(char *buf, int len)
{
    ...
    if (len < ssizeof(snp->sn_header)) {
        e_fatal(
            "invalid NOTIF: len = %d != %zd", len, sizeof(snp->sn_header)
        );
    }
    ...
    case SCTP_ASSOC_CHANGE:
        if (len < ssizeof(struct sctp_assoc_change)) {
            e_fatal(
                "invalid NOTIF assoc change: len = %d != %zd", len,
        ...
        e_trace(
            0, "state=%u, error=%u, instreams=%u, outstreams=%u)",
    ...
    case SCTP_SEND_FAILED:
        if (len < ssizeof(struct sctp_send_failed)) {
            e_fatal(
                "invalid NOTIF send_failed: len = %d != %zd", len,
        ...
        e_trace(
            0, "sendfailed: len=%u, err=%d", ssf->ssf_length, ssf->ssf_error
        );
        break;
    ...
    case SCTP_REMOTE_ERROR:
        if (len < ssizeof(struct sctp_remote_error)) {
            e_fatal(
                "invalid NOTIF remote_error: len = %d != %zd", len,
