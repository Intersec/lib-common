 ...
 */
void net_rctl_init_slots(
    int rate, int slots_nr, unsigned slots[static slots_nr]
);
#  else
...
void net_rctl_init(
    net_rctl_t *nonnull rctl, int rate,
    void (*nonnull cb)(net_rctl_t *nonnull)
);
#  ifdef __has_blocks
void net_rctl_init_blk(
    net_rctl_t *nonnull rctl, int rate, block_t nonnull blk
);
#  endif /* __has_blocks */
...
#endif /* !defined(IS_LIB_COMMON_NET_H) ||                                   \
          defined(IS_LIB_COMMON_NET_RATE_H)*/
