...
int sctp_connectx_ng(
    int fd, struct sockaddr *nonnull addrs, int count,
...
ssize_t sctp_sendv(
    int sd, const struct iovec *nullable iov, int iovlen,
...
#  ifndef __cplusplus
int sctp_getaddrs(
    int fd, int optnum, sctp_assoc_t id, struct sockaddr *nonnull addrs,
    int addr_size
);
#  endif
