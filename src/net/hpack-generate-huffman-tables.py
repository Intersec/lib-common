...
HuffManCodeTable = list[tuple[int, int, int, str]]


def get_license_as_c_comment() -> str:
    license_comment = str(inspect.getcomments(sys.modules[__name__]))
    license_comment = '\n' + license_comment + '\n'
    license_comment = (
        license_comment.replace('\n#', '\n/*')
        .replace('#\n', '*/\n')
        .replace('#', '*')
    )
...
def get_huffman_code_table_from(rfc_fn: str) -> HuffManCodeTable:
    def extract_huffman_code_table_text(rfc_fn: str) -> str:
        with open(rfc_fn, 'r') as f:
...
def get_huffman_encoding_table_as_c_array(
    rfc_fn: str, tbl_elem_t: str, tbl_name: str
) -> str:
    ...
    content = '{\n  ' + ',\n  '.join(lines) + '\n}'
    return f'{tbl_elem_t} {tbl_name}[{len(code_tab) - 1} + 1] = {content};'


def gen_table_for_encoding(
    rfc_fn: str,
    out_fn: str | None,
    hdr_lines: str,
    tbl_elem_t: str,
...
# Use a type defined as a string for the forward declaration of type Tree
TreeValue: TypeAlias = 'str | int | bool | Tree | None'
...
class Tree(UserDict[TreeKey, TreeValue]):
    ...
    def __missing__(self, key: str) -> Tree:
        ...
        if key == '*':
            ...
            value_prefix = cast(str, self['prefix']) + str(key)

        value['sym'] = None
        value['prefix'] = value_prefix
        value['final'] = ('1' * 7).startswith(value_prefix)
...
def get_huffman_tree(code_table: HuffManCodeTable) -> Tree:
    root = cast(Tree, Tree()['*'])
...
def walk_from(
    ...
    for bit in (int(ch) for ch in code_str):
        ...
        current = cast(Tree, current[bit])
        if current['sym'] is not None:
            # reached a leaf node : emit sym and continue from the root
            if current['sym'] != HUFFMAN_EOS:  # EOS
                sym.append(cast(int, current['sym']))
            else:
                error = True
            current = root

    final = not error and cast(bool, current['final'])
    n_next = current
    return (sym, final, error, n_next)


StTableState = list[dict[str, Any]]
StTable = list[StTableState]


def gen_huffman_st_table(tree: Tree, chunk_bits: int) -> StTable:
    def new_state(node: Tree) -> None:
        node['IDX'] = len(st_tab)
        st_tab.append(
            [
...
def get_huffman_st_table_as_c_array(
    rfc_fn: str, tbl_elem_t: str, tbl_name: str, chunk_bits: int
) -> str:
    def get_state_transitions_row(idx: int, state: StTableState) -> str:
        cols = []
        for chunk_vec in product('01', repeat=chunk_bits):
            chunk_str = ''.join(chunk_vec)
            chunk_code = int(chunk_str, 2)
            ...
            (
                next_idx,
                emitter,
            ) = trans['next']['IDX'], len(trans['sym']) > 0
    ...
    for idx, state in enumerate(st_tab):
        ...
        rows.append(
            f'/* node/state: [ {prefix:<10} ] {idx:3d} */\n'
            + get_state_transitions_row(idx, state)
        )

    content = '{\n  ' + ',\n  '.join(rows) + '\n}'
    return (
        f'{tbl_elem_t} {tbl_name}[{len(st_tab)}]'
        f'[{len(st_tab[0])}] = {content};'
    )


def gen_table_for_decoding(
    rfc_fn: str,
    out_fn: str | None,
    hdr_lines: str,
    tbl_elem_t: str,
    ...
    head = HDR_AUTOGENERATED
    license_comment = get_license_as_c_comment()
    c_array_dec = get_huffman_st_table_as_c_array(
        rfc_fn, tbl_elem_t, tbl_name, chunk_bits
    )
    content = '\n'.join(  # noqa: FLY002 (static-join-to-f-string)
        [head, license_comment, '', hdr_lines, '', c_array_dec, '']
    )
...
def options(args: list[str]) -> argparse.Namespace:
    op = argparse.ArgumentParser(
        description=(
            'Generate static Huffman encoding and decoding tables of'
            ' the HPACK algorithm'
        )
    )
    ...
    sp.add_argument(
        '-r', '--rfc', help='rfc 7541 file', required=True, dest='rfc_fn'
    )
    sp.add_argument(
        '-o', '--out', help='output file', dest='out_fn', default=None
    )
    sp.add_argument(
        '-H',
        '--header',
    ...
    sp.add_argument(
        '-t',
        '--elem-type',
        help='type of table element',
        dest='elem_t',
        default=DEF_ENCODE_ELEM_T,
    )
    sp.add_argument(
        '-n',
        '--name',
        help='name of table variable',
        dest='name',
        default=DEF_ENCODE_TABLE_NAME,
    )
...
def main(args: list[str]) -> None:
    def none_if_minus(fn: str) -> str | None:
