...
static sockunion_t *
t_sctp_sus_array(const qv_t(sctp_su) *sus, int *nullable olen)
{
...
static int sctp_ep_init_sock(int fd, size_t sndbuf)
{
    int nodelay = 1;

    assert(fd >= 0);

    RETHROW(sctp_enable_events(
        fd, SCTP_DATA_IO_EV | SCTP_ASSOCIATION_EV | SCTP_SEND_FAILURE_EV |
                SCTP_PEER_ERROR_EV | SCTP_ADDRESS_EV | SCTP_SHUTDOWN_EV
    ));
...
static int sctp_send_queued_msgs(sctp_conn_priv_t **pconn)
{
    ...
    dlist_for_each_entry(sctp_msg_t, msg, &conn->msgs, msg_list) {
        errno = 0;
        len = sctp_sendmsg(
            fd, msg->msg.data, msg->msg.len, NULL, 0, htonl(msg->ppid),
            msg_flags, stream_no, 0, 0
        );
        if (len < 0) {
            if (!ERR_RW_RETRIABLE(errno)) {
                logger_error(
                    conn->user_ctx.logger,
        ...
        logger_trace(
            conn->user_ctx.logger, 2, "send message to %d succeed (%d bytes)",
            fd, len
        );
        sctp_msg_delete(&msg);
    }
...
static int sctp_handle_notification(sctp_conn_priv_t **pconn)
{
    ...
    if (conn->rbuf.len < ssizeof(sn->sn_header)) {
        logger_error(
            conn->user_ctx.logger, "invalid NOTIF: len = %d < %zd",
    ...
    case SCTP_ASSOC_CHANGE: {
        ...
        logger_trace(
            conn->user_ctx.logger, 2,
            "got notif ASSOC_CHANGE with "
    ...
    case SCTP_SEND_FAILED: {
        ...
        logger_error(
            conn->user_ctx.logger, "got send failed error %d with peer `%pL'",
    ...
    case SCTP_REMOTE_ERROR: {
        ...
        logger_error(
            conn->user_ctx.logger, "got remote error %d with peer `%pL'",
    ...
    default:
        logger_trace(
            conn->user_ctx.logger, 2, "got notif %d", sn->sn_header.sn_type
        );
        break;
    }
...
static int sctp_receive_full_msg(sctp_conn_priv_t **pconn)
{
    ...
    for (;;) {
        errno = 0;
        len = sctp_recvmsg(
            fd, sb_grow(ibuf, BUFSIZ), BUFSIZ, NULL, NULL, &sinfo, &msg_flags
        );
        if (len < 0) {
    ...
    if (unlikely(msg_flags & MSG_CTRUNC)) {
        logger_error(
            conn->user_ctx.logger, "ancillary data truncated, "
    ...
    logger_trace(
        conn->user_ctx.logger, 2, "SCTP received %d bytes from fd: %d",
        ibuf->len, fd
    );
    conn->reset_rbuf = true;
...
static int sctp_conn_on_event_in(sctp_conn_priv_t **pconn)
{
    ...
    if (data_received) {
        if (conn->on_data_cb(&conn->user_ctx, NULL, true) < 0) {
            logger_warning(
                conn->user_ctx.logger, "user on_data fn returned an error"
            );
            goto error;
        }
    }
...
static int sctp_connecting(el_t evh, int fd, short ev, el_data_t priv)
{
    ...
    if (ret < 0 || err) {
        logger_trace(
            conn->user_ctx.logger, 4,
            "error returned while connecting. Using error callback"
        );
        if (len != sizeof(err)) {
            logger_warning(
                conn->user_ctx.logger,
                "unexpected error code length (%d != %ld)", len, sizeof(err)
            );
        }
    ...
    if (conn->on_connect_cb(&conn->user_ctx, true, 0) < 0) {
        logger_error(
            conn->user_ctx.logger, "user on_connect_cb returned an error"
        );
        sctp_conn_close_priv(&conn);
        return -1;
    }
...
static int sctp_srv_on_event(el_t evh, int fd, short ev, el_data_t priv)
{
    ...
    while ((sock = acceptx(fd, O_NONBLOCK)) >= 0) {
        ...
        logger_name = t_lstr_cat3(
            user_ctx->entity_id, LSTR("-"), t_lstr_fmt("%d", sock)
        );

        logger = logger_new(&sctp_logger_g, logger_name, LOG_INHERITS, 0);

        if (getpeername(sock, &local.sa, &size) < 0) {
            logger_error(
                logger, "cannot get peer name for `%*pM': %m",
        ...
        if ((ret = getnameinfo(
                 &local.sa, size, host, NI_MAXHOST, serv, NI_MAXSERV,
                 NI_NUMERICHOST | NI_NUMERICSERV
             )) != 0)
        {
            logger_error(
                logger, "cannot get peer informations for `%*pM': %s",
                LSTR_FMT_ARG(user_ctx->entity_id), gai_strerror(ret)
            );
            close(sock);
            logger_delete(&logger);
            return -1;
        ...
        port = atoi(serv);
        logger_info(
            logger, "connection for `%*pM' from %s:%d",
            LSTR_FMT_ARG(user_ctx->entity_id), host, port
        );

        conn = sctp_conn_priv_new();
        conn->evh =
            el_fd_register(sock, true, POLLIN, sctp_conn_on_event, conn);
...
static void sctp_conn_close_priv(sctp_conn_priv_t **pconn)
{
    ...
    logger_info(
        conn->user_ctx.logger, "closing connection with %pL",
...
sctp_conn_t *sctp_connect(
    qv_t(lstr) source_addrs, qv_t(lstr) dest_addrs, uint16_t port,
    size_t sndbuf, lstr_t entity_id, void *priv,
    sctp_on_connect_f on_connect_cb, sctp_on_data_f on_data_cb,
    sctp_on_disconnect_f on_disconnect_cb
)
{
    t_scope;
    ...
    if (source_addrs.len) {
        ...
        if (bindx(
                fd, sus_array, sus.len, SOCK_SEQPACKET, IPPROTO_SCTP,
                O_NONBLOCK
            ) < 0)
        {
            logger_error(
                logger, "SCTP bind on address(es) %pL failed: %m", &s
            );
            close(fd);
            return NULL;
        }
        logger_info(
            logger, "outgoing connection: bind on `%pL` succeed for %pL", &s,
            &entity_id
        );
        qv_clear(&sus);
    }
    ...
    conn->user_ctx.logger =
        logger_new(&sctp_logger_g, entity_id, LOG_INHERITS, 0);
...
sctp_conn_t *sctp_listen(
    const lstr__array_t *addrs, int port, size_t sndbuf, lstr_t entity_id,
    void *priv, sctp_on_accept_f on_accept_cb, sctp_on_data_f on_data_cb,
    sctp_on_disconnect_f on_disconnect_cb
)
{
    t_scope;
    ...
    if (bindx(
            fd, t_sctp_sus_array(&sus, NULL), sus.len, SOCK_SEQPACKET,
            IPPROTO_SCTP, O_NONBLOCK
        ) < 0)
    {
        logger_error(
            logger, "SCTP bind on address %*pM failed: %m",
    ...
    if (listen(fd, SOMAXCONN) < 0) {
        logger_error(
            logger, "SCTP listen on address %*pM failed: %m",
            LSTR_FMT_ARG(sus_str)
        );
        close(fd);
        return NULL;
    ...
    logger_info(
        logger, "listening on SCTP address(es): %*pM", LSTR_FMT_ARG(sus_str)
    );

    conn = sctp_conn_priv_new();
    conn->evh = el_fd_register(fd, true, POLLIN, sctp_srv_on_event, conn);
    ;
...
void sctp_send_msg(
    sctp_conn_t *pub_ctx, lstr_t payload, uint32_t payload_protocol_id
)
{
