...
static inline bool qhhash_str_equal(
    const qhhash_t *qhh, const qhash_t *qh, const char *s1, const char *s2
)
{
...
static inline bool qhhash_lstr_equal(
    const qhhash_t *qhh, const qhash_t *qh, const lstr_t *s1, const lstr_t *s2
)
{
    return qhash_lstr_equal(NULL, s1, s2);
}
static inline bool qhhash_ptr_equal(
    const qhhash_t *qhh, const qhash_t *qh, const void *ptr1, const void *ptr2
)
{
...
#define __QHH_FOREACH_BUCKET(it, qhh, do_it, ...)                            \
    do {                                                                     \
...
#define qhhash_for_each_pos(pos, hh)                                         \
    for (uint64_t __b_##pos = 0; __b_##pos < countof((hh)->buckets);         \
         __b_##pos++)                                                        \
        for (uint64_t __##pos##_priv =                                       \
             ...
             __##pos##_priv = qhash_scan(                                    \
                                  &(hh)->buckets[__b_##pos].qm.qh,           \
...
#define __QHH_BASE(pfx, name, hpfx, bucket_count, ckey_t, pkey_t, hf)        \
    ...
    typedef struct pfx##_t {                                                 \
        ...
        pfx##_bucket_t buckets[bucket_count];                                \
                                                                             ...
                                                                             \
    __attr_unused__ static inline pfx##_t *pfx##_init(                       \
        pfx##_t *qhh, bool chashes, mem_pool_t *nullable mp                  \
    )                                                                        \
    {                                                                        \
        p_clear(&qhh->hdr, 1);                                               \
        for (int it = 0; it < countof(qhh->buckets); it++) {                 \
        ...
        return qhh;                                                          \
                                                                             ...
                                                                             \
    __attr_unused__ static inline size_t pfx##_memory_footprint(             \
        const pfx##_t *qhh                                                   \
    )                                                                        \
    {                                                                        \
        size_t size = 0;                                                     \
        ...
        return size;                                                         \
                                                                             ...
                                                                             \
    __attr_unused__ static inline void pfx##_wipe(pfx##_t *qhh)              \
    {                                                                        \
        __QHH_FOREACH_BUCKET(i, qhh, qhash_wipe);                            \
    }                                                                        \
                                                                             \
    __attr_unused__ static inline void pfx##_clear(pfx##_t *qhh)             \
    {                                                                        \
        __QHH_FOREACH_BUCKET(i, qhh, qhash_clear);                           \
        qhh->hdr.len = 0;                                                    \
    }                                                                        \
                                                                             \
    __attr_unused__ static inline void pfx##_set_minsize(                    \
        pfx##_t *qhh, uint64_t minsize                                       \
    )                                                                        \
    {                                                                        \
        ...
        for (int it = 0; it < countof(qhh->buckets); it++) {                 \
            qhash_set_minsize(&qhh->buckets[it].qm.qh, minsize);             \
                                                                             ...
                                                                             \
    __attr_unused__ static inline void pfx##_del_at(                         \
        pfx##_t *qhh, uint64_t pos                                           \
    )                                                                        \
    {                                                                        \
        ...
        qhh->hdr.len -= old_len - bucket->qh.hdr.len;                        \
                                                                             ...
                                                                             \
    __attr_unused__ static inline pkey_t pfx##_key_p(                        \
        const pfx##_t *qhh, uint64_t pos                                     \
    )                                                                        \
    {                                                                        \
        ...
        return &__QHH_BUCKET(qhh, pos)->qm.keys[__QHH_POS(qhh, pos)];        \
                                                                             ...
                                                                             \
    __attr_unused__ static inline uint32_t *pfx##_hash_p(                    \
        const pfx##_t *qhh, uint64_t pos                                     \
    )                                                                        \
    {                                                                        \
        pos &= ~(uint64_t)QHASH_COLLISION;                                   \
        return &__QHH_BUCKET(qhh, pos)->qm.hashes[__QHH_POS(qhh, pos)];      \
                                                                             ...
                                                                             \
    __attr_unused__ static inline uint64_t pfx##_len(const pfx##_t *qhh)     \
    {                                                                        \
        return qhh->hdr.len;                                                 \
    }                                                                        \
                                                                             \
    __attr_unused__ static inline uint32_t pfx##_hash(                       \
        const pfx##_t *qhh, ckey_t key                                       \
    )                                                                        \
    {                                                                        \
        return hf(&qhh->hdr, key);                                           \
    }                                                                        \
                                                                             \
    __attr_unused__ static inline uint32_t pfx##__hash(                      \
        const qhash_t *h, ckey_t key                                         \
    )                                                                        \
    {                                                                        \
        const pfx##_bucket_t *bucket =                                       \
            container_of(h, const pfx##_bucket_t, qm.qh);                    \
                                                                             \
        bucket -= bucket->pos;                                               \
        return pfx##_hash(                                                   \
            container_of(bucket, const pfx##_t, buckets[0]), key             \
        );                                                                   \
    }

#define __QHH_EQUAL(pfx, name, hpfx, ckey_t, ef)                             \
    __attr_unused__ static inline bool pfx##_equal(                          \
        const pfx##_t *qhh, const qhash_t *qh, ckey_t a, ckey_t b            \
    )                                                                        \
    {                                                                        \
        return ef(&qhh->hdr, qh, a, b);                                      \
    }                                                                        \
    __attr_unused__ static inline bool pfx##__equal(                         \
        const qhash_t *h, ckey_t a, ckey_t b                                 \
    )                                                                        \
    {                                                                        \
        const pfx##_bucket_t *bucket =                                       \
            container_of(h, const pfx##_bucket_t, qm.qh);                    \
        ...
        bucket -= bucket->pos;                                               \
        return pfx##_equal(                                                  \
            container_of(bucket, const pfx##_t, buckets[0]), h, a, b         \
        );                                                                   \
    }

#define __QHH_FIND(pfx, name, hpfx, ckey_t)                                  \
    __attr_unused__ static inline int64_t pfx##_find_h(                      \
        pfx##_t *qhh, uint32_t h, ckey_t key                                 \
    )                                                                        \
    {                                                                        \
        ...
        return pos;                                                          \
                                                                             ...
                                                                             \
    __attr_unused__ static inline int64_t pfx##_find(                        \
        pfx##_t *qhh, ckey_t key                                             \
    )                                                                        \
    {                                                                        \
        return pfx##_find_h(qhh, pfx##_hash(qhh, key), key);                 \
    }                                                                        \
                                                                             \
    __attr_unused__ static inline int64_t pfx##_find_safe_h(                 \
        const pfx##_t *qhh, uint32_t h, ckey_t key                           \
    )                                                                        \
    {                                                                        \
        uint64_t bid = h % countof(qhh->buckets);                            \
        uint64_t pos;                                                        \
        ...
        return pos;                                                          \
                                                                             ...
                                                                             \
    __attr_unused__ static inline int64_t pfx##_find_safe(                   \
        const pfx##_t *qhh, ckey_t key                                       \
    )                                                                        \
    {                                                                        \
...
#define __QHH_ADD(pfx, name, hpfx, key_t)                                    \
    __attr_unused__ static inline uint64_t pfx##_put_h(                      \
        pfx##_t *qhh, uint32_t h, key_t key, uint32_t fl                     \
    )                                                                        \
    {                                                                        \
        uint64_t bid = h % countof(qhh->buckets);                            \
        uint64_t pos;                                                        \
    ...
    __attr_unused__ static inline uint64_t pfx##_put(                        \
        pfx##_t *qhh, key_t key, uint32_t fl                                 \
    )                                                                        \
    {                                                                        \
        return pfx##_put_h(qhh, pfx##_hash(qhh, key), key, fl);              \
    }                                                                        \
                                                                             \
    __attr_unused__ static inline int pfx##_add_h(                           \
        pfx##_t *qhh, uint32_t h, key_t key                                  \
    )                                                                        \
    {                                                                        \
        uint64_t bid = h % countof(qhh->buckets);                            \
    ...
    __attr_unused__ static inline int pfx##_add(pfx##_t *qhh, key_t key)     \
    {                                                                        \
        return pfx##_add_h(qhh, pfx##_hash(qhh, key), key);                  \
    }                                                                        \
                                                                             \
    __attr_unused__ static inline int pfx##_replace_h(                       \
        pfx##_t *qhh, uint32_t h, key_t key                                  \
    )                                                                        \
    {                                                                        \
        uint64_t bid = h % countof(qhh->buckets);                            \
        int ret;                                                             \
                                                                             \
        ret = hpfx##_reserve_int(                                            \
            &qhh->buckets[bid].qm, &h, key, QHASH_OVERWRITE                  \
        );                                                                   \
        if (!(ret & QHASH_COLLISION)) {                                      \
            qhh->hdr.len++;                                                  \
        }                                                                    \
    ...
    __attr_unused__ static inline int pfx##_replace(pfx##_t *qhh, key_t key) \
    {                                                                        \
...
#define __QHH_IKEY(sfx, pfx, name, key_t, bucket_count)                      \
    __QHH_BASE(                                                              \
        pfx, name, qh_u##sfx, bucket_count, key_t const, key_t *,            \
        qhhash_hash_u##sfx                                                   \
    );                                                                       \
...
#define __QHH_PKEY(                                                          \
    pfx, name, qhc_t, bkey_t, ckey_t, key_t, hf, ef, bucket_count            \
)                                                                            \
    ...
    qhc_t(qhh_##name, bkey_t, pfx##__hash, pfx##__equal);                    \
                                                                             \
    __QHH_BASE(                                                              \
        pfx, name, qh_qhh_##name, bucket_count, ckey_t *, key_t **, hf       \
    );                                                                       \
...
#define __QHM_ADD(pfx, name, hpfx, key_t, val_t)                             \
    __attr_unused__ static inline val_t *pfx##_value_p(                      \
        const pfx##_t *qhh, uint64_t pos                                     \
    )                                                                        \
    {                                                                        \
        pos &= ~(uint64_t)QHASH_COLLISION;                                   \
        return &__QHH_BUCKET(qhh, pos)->qm.values[__QHH_POS(qhh, pos)];      \
                                                                             ...
                                                                             \
    __attr_unused__ static inline uint64_t pfx##_put_h(                      \
        pfx##_t *qhh, uint32_t h, key_t key, val_t v, uint32_t fl            \
    )                                                                        \
    {                                                                        \
        uint64_t bid = h % countof(qhh->buckets);                            \
        uint64_t pos;                                                        \
    ...
    __attr_unused__ static inline uint64_t pfx##_put(                        \
        pfx##_t *qhh, key_t key, val_t v, uint32_t fl                        \
    )                                                                        \
    {                                                                        \
        return pfx##_put_h(qhh, pfx##_hash(qhh, key), key, v, fl);           \
    }                                                                        \
                                                                             \
    __attr_unused__ static inline uint64_t pfx##_reserve_h(                  \
        pfx##_t *qhh, uint32_t h, key_t key, uint32_t fl                     \
    )                                                                        \
    {                                                                        \
        uint64_t bid = h % countof(qhh->buckets);                            \
    ...
    __attr_unused__ static inline uint64_t pfx##_reserve(                    \
        pfx##_t *qhh, key_t key, uint32_t fl                                 \
    )                                                                        \
    {                                                                        \
        return pfx##_reserve_h(qhh, pfx##_hash(qhh, key), key, fl);          \
                                                                             ...
                                                                             \
    __attr_unused__ static inline int pfx##_add_h(                           \
        pfx##_t *qhh, uint32_t h, key_t key, val_t v                         \
    )                                                                        \
    {                                                                        \
        return (int)pfx##_put_h(qhh, h, key, v, 0) >> 31;                    \
    }                                                                        \
    __attr_unused__ static inline int pfx##_add(                             \
        pfx##_t *qhh, key_t key, val_t v                                     \
    )                                                                        \
    {                                                                        \
    ...
    __attr_unused__ static inline int pfx##_replace(                         \
        pfx##_t *qhh, key_t key, val_t v                                     \
    )                                                                        \
    {                                                                        \
...
#define __QHM_IKEY(sfx, pfx, name, key_t, val_t, bucket_count)               \
    qm_k##sfx##_t(qhm_##name, val_t);                                        \
                                                                             \
    __QHH_BASE(                                                              \
        pfx, name, qm_qhm_##name, bucket_count, key_t const, key_t *,        \
        qhhash_hash_u##sfx                                                   \
    );                                                                       \
...
#define __QHM_PKEY(                                                          \
    pfx, name, qmc_t, bkey_t, ckey_t, key_t, val_t, hf, ef, bucket_count     \
)                                                                            \
    static inline uint32_t pfx##__hash(const qhash_t *h, ckey_t *);          \
    static inline bool pfx##__equal(const qhash_t *h, ckey_t *, ckey_t *);   \
    qmc_t(qhm_##name, bkey_t, val_t, pfx##__hash, pfx##__equal);             \
                                                                             \
    __QHH_BASE(                                                              \
        pfx, name, qm_qhm_##name, bucket_count, ckey_t *, key_t **, hf       \
    );                                                                       \
...
#define qhh_kptr_t(name, bucket_count, key_t, hf, ef)                        \
    __QHH_PKEY(                                                              \
        qhh_##name, name, qh_kptr_t, key_t, key_t const, key_t, hf, ef,      \
        bucket_count                                                         \
    )
#define qhh_kptr_ckey_t(name, bucket_count, key_t, hf, ef)                   \
    __QHH_PKEY(                                                              \
        qhh_##name, name, qh_kptr_ckey_t, key_t, key_t const, key_t const,   \
        hf, ef, bucket_count                                                 \
    )
...
#define qhh_init(name, qhh, chashes) qhh_##name##_init(qhh, chashes, NULL)
...
#define qhh_find_safe_h(name, qhh, h, key)                                   \
    qhh_##name##_find_safe_h(qhh, h, key)
...
#define __qhh_put_h(name, qhh, h, key, fl)                                   \
    qhh_put_h(name, (qhh), (h), (key), (fl))
...
#  define qhh_for_each_pos(name, pos, hh)                                    \
      STATIC_ASSERT(                                                         \
          __builtin_types_compatible_p(typeof(hh), qhh_t(name) *) ||         \
          __builtin_types_compatible_p(typeof(hh), const qhh_t(name) *)      \
      );                                                                     \
...
#define qhm_kvec_t(name, bucket_count, key_t, val_t, hf, ef)                 \
    __QHM_VKEY(                                                              \
        qhm_##name, name, key_t const, key_t, val_t, hf, ef, bucket_count    \
    )
#define qhm_kptr_t(name, bucket_count, key_t, val_t, hf, ef)                 \
    __QHM_PKEY(                                                              \
        qhm_##name, name, qm_kptr_t, key_t, key_t const, key_t, val_t, hf,   \
        ef, bucket_count                                                     \
    )
#define qhm_kptr_ckey_t(name, bucket_count, key_t, val_t, hf, ef)            \
    __QHM_PKEY(                                                              \
        qhm_##name, name, qm_kptr_ckey_t, key_t, key_t const, key_t const,   \
        val_t, hf, ef, bucket_count                                          \
    )
...
#define qhm_init(name, qhm, chashes) qhm_##name##_init(qhm, chashes, NULL)
...
#define qhm_find_safe_h(name, qhm, h, key)                                   \
    qhm_##name##_find_safe_h(qhm, h, key)
#define qhm_put(name, qhm, key, v, fl) qhm_##name##_put(qhm, key, v, fl)
#define qhm_put_h(name, qhm, h, key, v, fl)                                  \
    qhm_##name##_put_h(qhm, h, key, v, fl)
#define __qhm_put(name, qhm, key, v, fl)                                     \
    qhm_put(name, (qhm), (key), (v), (fl))
#define __qhm_put_h(name, qhm, h, key, v, fl)                                \
    qhm_put_h(name, (qhm), (h), (key), (v), (fl))
...
#define qhm_replace_h(name, qhm, h, key, v)                                  \
    qhm_##name##_replace_h(qhm, h, key, v)
...
#define _qhm_get_def(                                                        \
    name, _qh, _def, _qhm_modifier, _address_of_operator, _qhm_find,         \
    qhm_value_func, ...                                                      \
)                                                                            \
    ({                                                                       \
        ...
        __ghp_pos >= 0 ? qhm_value_func(name, __gqh, __ghp_pos) : __def;     \
    })
...
#define qhm_get_def_safe(name, _qhm, key, def)                               \
    _qhm_get_def(                                                            \
        name, (_qhm), (def), const, , qhm_find_safe, qhm_value, (key)        \
    )
#define qhm_get_def_safe_h(name, _qhm, h, key, def)                          \
    _qhm_get_def(                                                            \
        name, (_qhm), (def), const, , qhm_find_safe_h, qhm_value, (h), (key) \
    )
...
#define qhm_get_def_p_h(name, _qhm, h, key, def)                             \
    _qhm_get_def(                                                            \
        name, (_qhm), (def), , &, qhm_find_h, qhm_value_p, (h), (key)        \
    )
#define qhm_get_def_safe_p(name, _qhm, key, def)                             \
    _qhm_get_def(                                                            \
        name, (_qhm), (def), const, &, qhm_find_safe, qhm_value_p, (key)     \
    )
#define qhm_get_def_safe_p_h(name, _qhm, h, key, def)                        \
    _qhm_get_def(                                                            \
        name, (_qhm), (def), const, &, qhm_find_safe_h, qhm_value_p, (h),    \
        (key)                                                                \
    )
...
#  define qhm_for_each_pos(name, pos, hh)                                    \
      STATIC_ASSERT(                                                         \
          __builtin_types_compatible_p(typeof(hh), qhm_t(name) *) ||         \
          __builtin_types_compatible_p(typeof(hh), const qhm_t(name) *)      \
      );                                                                     \
      qhhash_for_each_pos(pos, hh)
#endif
...
#define qhm_reserve(name, qhm, key, fl) qhm_##name##_reserve(qhm, key, fl)
