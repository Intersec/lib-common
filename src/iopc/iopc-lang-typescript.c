...
static void parse_iface_path(
    const char *iface_fullname, const char **iface_name,
    ...
    *iface_path = NULL;
    ...
    if (ps_get_ps_lastchr_and_skip(&ps, '.', &ps_path) >= 0) {
        ...
        *iface_path = iopc_path_parse(LSTR_PS_V(&ps_path), &err);
        if (!*iface_path) {
            e_panic(
                "cannot parse path of interface %s: %*pM", iface_fullname,
                SB_FMT_ARG(&err)
            );
        }
    } else {
...
static void
t_iopc_dump_import(sb_t *buf, iopc_path_t *dep_name, qh_t(cstr) *imported)
{
    ...
    sb_addf(
        buf, "import * as %s from 'iop/%s.iop';\n", import_name,
        t_pp_path(dep_name)
    );
}
...
static void iopc_dump_imports(sb_t *buf, iopc_pkg_t *pkg)
{
    ...
    if (import_coll) {
        sb_adds(
            buf, "import { IopCollection } from 'iop/backbone/collection';\n"
        );
    }
    ...
    sb_addf(
        buf, "import JSON from 'json/%s.iop.json';\n", t_pp_path(pkg->name)
    );
    ...
    tab_for_each_entry(st, &pkg->structs) {
        tab_for_each_entry(attr, &st->attrs) {
            ...
            tab_for_each_entry(arg, &attr->args) {
                ...
                if (iface_path &&
                    !strequal(
                        pkg->name->slash_path, iopc_path_slash(iface_path)
                    ))
                {
...
static void iopc_dump_package_member(
    sb_t *buf, const iopc_pkg_t *pkg, const iopc_pkg_t *member_pkg,
    iopc_path_t *member_path, const char *member_name
)
{
    if (pkg != member_pkg) {
        t_scope;
...
static void iopc_dump_enum(
    sb_t *buf, const char *indent, const iopc_pkg_t *pkg,
...
)
{
    ...
    sb_addf(
        buf,
        "%sexport interface %s_ModelIf extends "
        "Enumeration<%s_Str, %s_Int> {\n",
        indent, en->name, en->name, en->name
    );
    tab_for_each_entry(field, &en->values) {
    ...
    sb_addf(
        buf,
        "%sexport const %s_Model: %s_ModelIf "
        "= enumeration<%s_Str, %s_Int>('%s.%s') as any;\n",
        indent, en->name, en->name, en->name, en->name, pp_dot(pkg->name),
        en->name
    );
}
...
static void iopc_dump_field_basetype(
...
static void iopc_dump_field_type(
    sb_t *buf, const iopc_pkg_t *pkg, const iopc_field_t *field,
    unsigned flags
)
{
    const char *suffix = (flags & USE_PARAM)   ? "_ModelParam"
                         : (flags & USE_MODEL) ? "_Model"
                                               : NULL;
...
static void iopc_dump_struct(
    sb_t *buf, const char *indent, const iopc_pkg_t *pkg, iopc_struct_t *st,
    const char *st_name
)
{
    ...
    sb_addf(
        buf, "%sconst %s_fullname = '%s%s%s.%s';\n", indent, st_name,
        pp_dot(pkg->name), st->iface ? "." : "",
    ...
    tab_for_each_entry(field, &st->fields) {
        sb_addf(buf, "%s    ", indent);
        iopc_dump_field(
            buf, pkg, field, OBJECT_FIELD | DEFVAL_AS_OPT | USE_PARAM
        );
    ...
    if (iopc_is_class(st->type)) {
        sb_addf(
            buf, "<Param extends %s_ModelParam = %s_ModelParam>", st_name,
            st_name
        );
    }
    sb_adds(buf, " extends ");
    ...
    tab_for_each_entry(attr, &st->attrs) {
        ...
        tab_for_each_entry(arg, &attr->args) {
            ...
            if (iface_path &&
                !strequal(pkg->name->slash_path, iopc_path_slash(iface_path)))
            {
                t_scope;
    ...
    if (no_ic_queries) {
        sb_addf(
            buf, "%s    declare public icQuery: (rpc: never) => never", indent
        );
    }
    ...
    sb_addf(buf, "%s};\n", indent);
    sb_addf(
        buf, "%sregisterModel(%s_Model, %s_fullname);\n", indent, st_name,
        st_name
    );
    if (iopc_should_dump_coll(st)) {
        if (iopc_is_class(st->type)) {
            sb_addf(
                buf,
                "%sexport class %s_Collection<Model extends "
                "%s_Model = %s_Model> extends ",
            ...
            sb_addf(
                buf, "%s    declare public icQuery: Model['icQuery'];\n",
                indent
            );
        } else {
            sb_addf(
                buf,
                "%sexport class %s_Collection extends "
                "IopCollection<%s_Model> {\n",
                indent, st_name, st_name
            );
            sb_addf(
                buf, "%s    declare public icQuery: %s_Model['icQuery'];\n",
                indent, st_name
            );
        }
        sb_addf(buf, "%s};\n", indent);
        sb_addf(
            buf,
            "%sregisterCollection<%s_Model>(%s_Collection, "
            "%s_fullname);\n",
            indent, st_name, st_name, st_name
        );
    }
...
)
{
    ...
    sb_addf(
        buf, "%sconst %s_fullname = '%s.%s';\n", indent, st_name,
        pp_dot(pkg->name), st_name
    );
    ...
    tab_for_each_entry(field, &st->fields) {
        ...
        sb_adds(buf, "{ ");
        iopc_dump_field(buf, pkg, field, OBJECT_FIELD | USE_PARAM | IN_UNION);
        sb_adds(buf, " }");

        if (field->kind == IOP_T_UNION || field->kind == IOP_T_STRUCT) {
            sb_addf(buf, "\n%s    | { ", indent);
            iopc_dump_field(
                buf, pkg, field, OBJECT_FIELD | USE_MODEL | IN_UNION
            );
            sb_adds(buf, " }");
        }
    ...
    sb_addf(
        buf,
        "%sexport class %s_Model extends UnionModel<%s_Keys, %s_ModelPairs, "
        "%s_ModelParam> {\n",
    ...
    sb_addf(
        buf,
        "%sexport class %s_Collection extends IopCollection<%s_Model> { };\n",
        indent, st_name, st_name
    );
    sb_addf(
        buf, "%sregisterCollection(%s_Collection, %s_fullname);\n", indent,
        st_name, st_name
    );
}
...
static void iopc_dump_rpc_fun_struct(
    sb_t *buf, const iopc_pkg_t *pkg, const iopc_fun_t *rpc, const char *type,
...
)
{
    if (iopc_fun_struct_is_void(fun_st)) {
        ...
        if (fun_st->is_anonymous) {
            ...
            iopc_dump_struct(
                buf, "        ", pkg, fun_st->anonymous_struct,
            ...
            sb_adds(buf, ";\n");
            sb_addf(
                buf, "        export type %s%s_ModelParam = ", rpc->name, type
            );
            iopc_dump_field_basetype(buf, pkg, fun_st->existing_struct, NULL);
            sb_adds(buf, "_ModelParam;\n");
        }
...
static void
iopc_dump_rpc(sb_t *buf, const iopc_pkg_t *pkg, const iopc_fun_t *rpc)
{
    t_scope;
    ...
    if (iopc_fun_struct_is_void(&rpc->arg)) {
        ...
        sb_addf(
            buf, "            args: %sArgs | %sArgs_ModelParam,\n", rpc->name,
            rpc->name
        );
    }
...
static void
iopc_dump_iface(sb_t *buf, const iopc_pkg_t *pkg, iopc_iface_t *iface)
{
    ...
    tab_enumerate(pos, rpc, &iface->funs) {
        if (pos == 0) {
            sb_addf(
                buf, "        export type IcQuery = %sIcQuery", rpc->name
            );
        } else {
            sb_addf(buf, "\n            & %sIcQuery", rpc->name);
        }
