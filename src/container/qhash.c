...
static bool qhash_should_resize(const qhash_t *qh)
{
    ...
    if (unlikely(
            (uint64_t)(hdr->len + qh->ghosts) * 3 >= (uint64_t)hdr->size * 2
        ))
    {
        return true;
    }
...
static void qhash_resize_start(qhash_t *qh)
{
    ...
    if (newsize > hdr->size) {
        ...
        qh->keys = mp_irealloc(
            hdr->mp, qh->keys, hdr->size * qh->k_size, newsize * qh->k_size,
            qh->k_align, MEM_RAW
        );
        if (qh->v_size) {
            qh->values = mp_irealloc(
                hdr->mp, qh->values, hdr->size * qh->v_size,
                newsize * qh->v_size, qh->v_align, MEM_RAW
            );
        }
        if (qh->h_size) {
            qh->hashes = mp_irealloc(
                hdr->mp, qh->hashes, hdr->size * 4, newsize * 4, 4, MEM_RAW
            );
        }
    }
    if (hdr->len) {
    ...
    hdr->size = newsize;
    hdr->bits =
        mp_new(hdr->mp, size_t, BITS_TO_ARRAY_LEN(size_t, 2 * newsize));
...
static void qhash_resize_done(qhash_t *qh)
{
    ...
    if (qh->old->size > size) {
        qh->keys = mp_irealloc(
            hdr->mp, qh->keys, qh->old->size * qh->k_size, size * qh->k_size,
            qh->k_align, MEM_RAW
        );
        if (qh->v_size) {
            qh->values = mp_irealloc(
                hdr->mp, qh->values, qh->old->size * qh->v_size,
                size * qh->v_size, qh->v_align, MEM_RAW
            );
        }
        if (qh->h_size) {
            qh->hashes = mp_irealloc(
                hdr->mp, qh->hashes, qh->old->size * 4, size * 4, 4, MEM_RAW
            );
        }
    }
...
void qhash_init(
    qhash_t *qh, uint16_t k_size, uint8_t k_align, uint16_t v_size,
    uint8_t v_align, bool doh, mem_pool_t *mp
)
{
    p_clear(qh, 1);
    qh->k_size = k_size;
...
#ifndef NDEBUG
    e_assert(
        panic, qh->ghosts != UINT32_MAX, "tried to clear a sealed hash table"
    );
#endif
