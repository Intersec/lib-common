...
static void http_iop_channel_wipe(http_iop_channel_t *channel)
{
    ...
    htlist_deep_clear(
        &channel->queries_waiting_conn, http_iop_msg_t, link,
        http_iop_msg_delete
    );
...
static void http_iop_start_msg(
    http_iop_channel_t *channel, http_iop_channel_remote_t *remote,
    httpc_t *httpc, http_iop_msg_t *msg, void *query_args
);

static int http_iop_get_ready_remote(
    http_iop_channel_t *channel, bool do_connection,
    http_iop_channel_remote_t **remote_ptr, httpc_t **httpc_ptr
)
{
...
static void http_iop_on_connection_ready(httpc_pool_t *pool, httpc_t *httpc)
{
    ...
    logger_trace(
        &_G.logger, 1, "connection on remote `%*pM` ready",
...
static void http_iop_on_connect_error(const httpc_t *httpc, int errnum)
{
    ...
    logger_trace(
        &_G.logger, 1, "connection on remote `%*pM` error: %s",
...
static void
http_iop_register_timeout_check(http_iop_channel_t *channel, int64_t timeout);
...
static void http_iop_timeout_queries(el_t el, data_t data)
{
    ...
    while (!htlist_is_empty(&channel->queries_waiting_conn)) {
        ...
        msg = htlist_first_entry(
            &channel->queries_waiting_conn, http_iop_msg_t, link
        );
        msg_expiry = timeval_addmsec(
            msg->query_time, channel->connection_timeout_msec
        );
        if (is_expired(&msg_expiry, &now, &time_left)) {
            logger_trace(
                &_G.logger, 1,
                "canceling query `%*pM`: connection "
                "timeout reached",
                LSTR_FMT_ARG(msg->rpc->name)
            );
            msg->cb(
                msg, IC_MSG_TIMEDOUT, (opt_http_code_t)OPT_NONE, NULL, NULL
            );
            htlist_pop_entry(
                &channel->queries_waiting_conn, http_iop_msg_t, link
            );
...
http_iop_on_query_done(httpc_query_t *http_query, httpc_status_t httpc_status)
{
    ...
    logger_trace(
        &_G.logger, 1, "query `%*pM` finished (%d)",
        LSTR_FMT_ARG(msg->rpc->name), httpc_status
    );
    if (http_query->qinfo) {
        logger_trace(
            &_G.logger, 2, "query `%*pM` HTTP code: %d",
            LSTR_FMT_ARG(msg->rpc->name), http_query->qinfo->code
        );
        logger_trace(
            &_G.logger, 3, "payload: `%*pM`", SB_FMT_ARG(&http_query->payload)
        );
    ...
    if (http_query->payload.len) {
        ...
        ctype = http_qhdr_find(
            http_query->qinfo->hdrs, http_query->qinfo->hdrs_len,
        ...
        if (ctype) {
            ...
            ps_skipspaces(&v);
            content_type_json =
                ps_startswithstr(&ctype->val, CONTENT_TYPE_JSON);
            content_type = LSTR_PS_V(&ctype->val);
        }

        if (content_type_json) {
            if (t_iop_junpack_ptr_ps(
                    msg->iop_env_ctx_guard.ctx, &ps, st, dest, 0, &err
                ) < 0)
            {
                logger_error(
                    &_G.logger,
                    "cannot unpack result of query "
                    "`%*pM`: %*pM",
                    LSTR_FMT_ARG(msg->rpc->name), SB_FMT_ARG(&err)
                );
            ...
            logger_error(
                ...
                "(code %d)",
                LSTR_FMT_ARG(msg->rpc->name), LSTR_FMT_ARG(content_type),
                http_query->qinfo->code
            );
        }
    } else if (
        httpc_status == HTTPC_STATUS_OK && http_query->qinfo &&
        http_code_is_successful(http_query->qinfo->code)
    )
    {
        ic_status = IC_MSG_INVALID;
        logger_error(
            &_G.logger, "invalid reply from server, empty payload "
...
static void http_iop_start_msg(
    http_iop_channel_t *channel, http_iop_channel_remote_t *remote,
    httpc_t *httpc, http_iop_msg_t *msg, void *args
)
{
    SB_1k(sb);
    SB_1k(query_data);
    ...
    if (channel->encode_url) {
        ...
        sb_addc(&sb, '/');
        sb_add_urlencode(
            &sb, msg->iface_alias->name.s, msg->iface_alias->name.len
        );
        sb_addc(&sb, '/');
        sb_add_urlencode(&sb, msg->rpc->name.s, msg->rpc->name.len);
    } else {
        sb_addf(
            &sb, "/%*pM/%*pM/%*pM", LSTR_FMT_ARG(remote->base_path),
            LSTR_FMT_ARG(msg->iface_alias->name), LSTR_FMT_ARG(msg->rpc->name)
        );
    }

    httpc_query_start_flags(
        &msg->query, HTTP_METHOD_POST, remote->pool.host, LSTR_SB_V(&sb),
        false
    );

    if (msg->user.len && msg->password.len) {
        httpc_query_hdrs_add_auth(&msg->query, msg->user, msg->password);
    } else if (channel->user.len && channel->password.len) {
        httpc_query_hdrs_add_auth(
            &msg->query, channel->user, channel->password
        );
    }
    ...
    logger_trace(
        &_G.logger, 1, "%*pM/%*pM: `%*pM`",
        LSTR_FMT_ARG(msg->iface_alias->name), LSTR_FMT_ARG(msg->rpc->name),
        SB_FMT_ARG(&query_data)
    );
...
http_iop_channel_create(const http_iop_channel_cfg_t *cfg, sb_t *err)
{
    ...
    res->iop_env = cfg->iop_env;
    res->connection_timeout_msec =
        OPT_DEFVAL(cfg->connection_timeout_msec, 10 * 1000);
    ...
    tab_for_each_entry(url, &cfg->urls) {
        ...
        remote->pool.host =
            lstr_fmt("%s:%d", remote->url.host, remote->url.port);
...
void http_iop_channel_cancel_messages(http_iop_channel_t *channel)
{
    while (!htlist_is_empty(&channel->queries_waiting_conn)) {
        ...
        msg->cb(msg, IC_MSG_CANCELED, (opt_http_code_t)OPT_NONE, NULL, NULL);
        htlist_pop_entry(
            &channel->queries_waiting_conn, http_iop_msg_t, link
        );
        http_iop_msg_delete(&msg);
...
void http_iop_query_(
    http_iop_channel_t *channel, http_iop_msg_t *msg, void *args
)
{
    http_iop_channel_remote_t *remote;
    httpc_t *httpc;
    ...
    if (http_iop_get_ready_remote(channel, true, &remote, &httpc) < 0) {
        ...
        if (!msg->args) {
            msg->args =
                mp_iop_dup_desc_flags_sz(NULL, msg->rpc->args, args, 0, NULL);
        }
