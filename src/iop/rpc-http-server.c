...
static int t_parse_soap(ichttp_query_t *iq, ichttp_cb_t **cbout, void **vout)
{
#define xr xmlr_g
#define XCHECK(expr) XMLR_CHECK(expr, goto xmlerror)
    ...
    if (pos < 0) {
        __ichttp_reply_soap_err_cst(
            ichttp_query_to_slot(iq), false, "unknown rpc"
        );
        goto error;
    }
    iq->cbe = *cbout = cbe = ichttp_cb_retain(tcb->impl.values[pos]);

    XCHECK(iop_xunpack_ptr_flags(
        xr, t_pool(), iq->iop_env_ctx_guard.ctx, cbe->fun->args, vout,
        tcb->unpack_flags
    ));
...
static int is_ctype_json(const httpd_qinfo_t *info)
{
    ...
    ctype =
        http_qhdr_find(info->hdrs, info->hdrs_len, HTTP_WKHDR_CONTENT_TYPE);
    if (ctype) {
...
int __t_ichttp_query_on_done_stage1(
    httpd_query_t *q, ichttp_cb_t **cbe, void **value, bool *soap
)
{
    ichttp_query_t *iq = obj_vcast(ichttp_query, q);
    httpd_trigger__ic_t *tcb =
        container_of(iq->trig_cb, httpd_trigger__ic_t, cb);
    ...
    *soap = false;
    ...
    if (ps_len(&url)) {
        ...
        if (is_ctype_json(q->qinfo)) {
            ...
            httpd_reject(
                obj_vcast(httpd_query, iq), NOT_ACCEPTABLE,
...
void __t_ichttp_query_on_done_stage2(
    httpd_query_t *q, ichttp_cb_t *cbe, void *value
)
{
    ichttp_query_t *iq = obj_vcast(ichttp_query, q);
    httpd_trigger__ic_t *tcb =
        container_of(iq->trig_cb, httpd_trigger__ic_t, cb);
    ic__hdr__t default_hdr = IOP_UNION_VA(
        ic__hdr, simple, .kind = LSTR_OPT(tcb->auth_kind),
        .payload = q->received_body_length, .source = LSTR("webservice"),
...
static void httpd_trigger__ic_cb(
    httpd_trigger_t *tcb, httpd_query_t *q, const httpd_qinfo_t *req
)
{
    httpd_trigger__ic_t *cb = container_of(tcb, httpd_trigger__ic_t, cb);
...
httpd_trigger__ic_t *httpd_trigger__ic_new(
    const iop_env_t *iop_env, const iop_mod_t *mod, const char *schema,
    unsigned szmax
)
{
...
static ichttp_cb_t *ichttp_register_function(
    httpd_trigger__ic_t *tcb, const iop_iface_alias_t *alias,
    const iop_rpc_t *fun, int32_t cmd, const ic_cb_entry_t *_cb
)
{
    ...
    e_assert_n(
        panic, qm_add(ichttp_cbs, &tcb->impl, &cb->name, cb), "RPC %s.%s",
        alias->name.s, fun->name.s
    );
    /* Register RPC URI (duplicates "cb") */
    e_assert_n(
        panic,
        qm_add(ichttp_cbs, &tcb->impl, &cb->name_uri, ichttp_cb_retain(cb)),
...
static void ichttp_serialize_soap(
    sb_t *sb, ichttp_query_t *iq, int cmd, const iop_struct_t *st,
    const void *v
)
{
    xmlpp_t pp;
    ...
    xmlpp_putattr(
        &pp, "xmlns:s", "http://schemas.xmlsoap.org/soap/envelope/"
    );
...
void __ichttp_reply(
    uint64_t slot, int cmd, const iop_struct_t *st, const void *v
)
{
    ...
    if (gzenc & HTTPD_ACCEPT_ENC_GZIP) {
        ...
        is_gzip = true;
    } else if (gzenc & HTTPD_ACCEPT_ENC_DEFLATE) {
    ...
    if (gzenc) {
        ...
        sb_add_compressed(
            out, buf.data, buf.len, Z_BEST_COMPRESSION, is_gzip
        );
    } else if (iq->json) {
...
void __ichttp_reply_soap_err(
    uint64_t slot, bool serverfault, const lstr_t *err
)
{
...
__attr_printf__(4, 5) static void __ichttp_reject(
    uint64_t slot, ichttp_query_t *iq, http_code_t rest_code, const char *fmt,
    ...
)
{
    SB_1k(err);
    ...
    if (iq->json) {
        ...
        httpd_reject_(
            obj_vcast(httpd_query, iq), rest_code, "%*pM", SB_FMT_ARG(&err)
        );
        __ichttp_err_ctx_clear();
    } else {
...
void __ichttp_reply_err(uint64_t slot, int err, const lstr_t *err_str)
{
    ...
    case IC_MSG_PROXY_ERROR:
        __ichttp_reject(
            slot, iq, HTTP_CODE_BAD_REQUEST, "query temporary refused"
        );
        break;
    case IC_MSG_INVALID:
    case IC_MSG_SERVER_ERROR:
        if (err_str && err_str->len) {
            __ichttp_reject(
                slot, iq, HTTP_CODE_BAD_REQUEST, "%*pM",
                LSTR_FMT_ARG(*err_str)
            );
        } else {
            __ichttp_reject(
                slot, iq, HTTP_CODE_BAD_REQUEST, "query refused by server"
            );
        }
        break;
    case IC_MSG_UNIMPLEMENTED:
        __ichttp_reject(
            slot, iq, HTTP_CODE_NOT_FOUND, "query not implemented by server"
        );
        break;
    }
}
...
void __ichttp_proxify(uint64_t slot, int cmd, const void *data, int dlen)
{
...

    {
        ...
        if (unlikely(
                iop_bunpack(
                    t_pool(), iq->iop_env_ctx_guard.ctx, st, v, ps, false
                ) < 0
            ))
        {
...
void __ichttp_forward_reply(
    ichannel_t *pxy_ic, uint64_t slot, int cmd, const void *res,
    const void *exn
)
{
    ichttp_query_t *iq = ichttp_slot_to_query(slot);
