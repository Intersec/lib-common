...
#  define IOP_CORE_OBJ_METHODS(type_t, ...) OBJECT_METHODS(type_t)

OBJ_CLASS(
    iop_core_obj, object, _IOP_CORE_OBJ_FIELDS, IOP_CORE_OBJ_METHODS, void
);
...
void *nullable _iop_core_obj_map_new_obj(
    const iop_core_obj_map_t *nonnull map, const void *nonnull iop_obj
);
const object_class_t *nullable _iop_core_obj_map_get_cls(
    const iop_core_obj_map_t *nonnull map, const void *nonnull iop_obj
);
void _iop_core_obj_map_register_cls(
...
void iop_core_obj_map_for_each_cls(
    const iop_core_obj_map_t *nonnull map, on_core_obj_cls_b nonnull on_cls
);
...
#  define IOP_CORE_OBJ_IMPL(map, cls_pfx, iop_cls_pfx, ...)                  \
      __VA_ARGS__ void cls_pfx##_register(                                   \
          const iop_struct_t *iop_cls, const object_class_t *cls             \
      )                                                                      \
      {                                                                      \
          e_assert(                                                          \
              panic, iop_class_is_a(iop_cls, &iop_cls_pfx##__s),             \
              "`%pL' is not a `%pL'", &iop_cls->fullname,                    \
              &iop_cls_pfx##__s.fullname                                     \
          );                                                                 \
          e_assert(                                                          \
              panic, cls_inherits(cls, obj_class(cls_pfx)),                  \
      ...
      {                                                                      \
          return cls_cast(                                                   \
              cls_pfx, RETHROW_P(_iop_core_obj_map_get_cls((map), desc))     \
          );                                                                 \
      }
