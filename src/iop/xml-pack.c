...
static void
xpack_struct(sb_t *, const iop_struct_t *, const void *, unsigned);
...
static void xpack_value(
    sb_t *sb, const iop_struct_t *desc, const iop_field_t *f, const void *v,
    unsigned flags
)
{
    ...
    if (is_class) {
        ...
        sb_addf(
            sb, " xsi:type=\"n:%*pM\">", LSTR_FMT_ARG(real_desc->fullname)
        );
    } else if (
    ...
    case IOP_T_I8:
        sb_addf(sb, "%i", *(int8_t *)v);
        break;
    case IOP_T_U8:
        sb_addf(sb, "%u", *(uint8_t *)v);
        break;
    case IOP_T_I16:
        sb_addf(sb, "%i", *(int16_t *)v);
        break;
    case IOP_T_U16:
        sb_addf(sb, "%u", *(uint16_t *)v);
        break;
    case IOP_T_I32:
    ...
    case IOP_T_ENUM:
        ...
        break;
    case IOP_T_U32:
        sb_addf(sb, "%u", *(uint32_t *)v);
        break;
    case IOP_T_I64:
        sb_addf(sb, "%jd", *(int64_t *)v);
        break;
    case IOP_T_U64:
        sb_addf(sb, "%ju", *(uint64_t *)v);
        break;
    case IOP_T_DOUBLE:
        d = *(double *)v;
        if (isinf(d)) {
...
static void xpack_struct(
    sb_t *sb, const iop_struct_t *desc, const void *v, unsigned flags
)
{
...
static void
xpack_class(sb_t *sb, const iop_struct_t *desc, const void *v, unsigned flags)
{
    qv_t(iop_struct) parents;
    const iop_struct_t *real_desc = *(const iop_struct_t **)v;

    e_assert(
        panic, !real_desc->class_attrs->is_abstract,
...
static void
xpack_union(sb_t *sb, const iop_struct_t *desc, const void *v, unsigned flags)
{
...
void iop_xpack(
    sb_t *sb, const iop_struct_t *desc, const void *v, bool verbose,
    bool with_enums
)
{
    unsigned flags = 0;
