...
lzo_get_varlen(pstream_t *in, unsigned u, unsigned mask)
{
    unsigned sz = u & mask;
...
static ALWAYS_INLINE int
lzo_copy_input(pstream_t *in, ostream_t *os, unsigned sz)
{
...
/* XXX: assumes sz <= 4 */
static ALWAYS_INLINE int
lzo_copy_input_small(pstream_t *in, ostream_t *os, unsigned sz)
{
...
static ALWAYS_INLINE int lzo_copy_backptr(
    ostream_t *os, const uint8_t *out_orig, unsigned back, unsigned sz
)
{
...
static ALWAYS_INLINE int
lzo_copy_backptr3(ostream_t *os, const uint8_t *out_orig, unsigned back)
{
    const uint8_t *src = os->b - back;
    uint8_t *dst = os->b;
...
__attr_flatten__ ssize_t
qlzo1x_decompress(void *out, size_t outlen, pstream_t in)
{
    return decompress(out, outlen, in, false);
}

__attr_flatten__ ssize_t
qlzo1x_decompress_safe(void *out, size_t outlen, pstream_t in)
{
