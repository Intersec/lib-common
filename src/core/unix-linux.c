...
#  define PF_FREEZE                                                          \
      0x00004000 /* this task is being frozen for suspend now                \
                  */
...
#  define PF_SPREAD_SLAB                                                     \
      0x02000000                  /* Spread some slab caches over cpuset     \
                                   */
#  define PF_MEMPOLICY 0x10000000 /* Non-default NUMA mempolicy */
#  define PF_MUTEX_TESTER                                                    \
      0x20000000 /* Thread belongs to the rt mutex tester */
...
static int psinfo_read_proc(pid_t pid, sb_t *output, sb_t *buf)
{
    ...
    while (p && *p != '\0') {
        ...
        case 6: /*tty_nr %d
            ...
            break;
        case 7: /*tpgid %d
                  The process group ID of the process which currently owns the
                  tty that the process is connected to.*/
            break;
        case 8: /*flags %lu
...
#  define CHECK_KERNEL_FLAG(flag, txt)                                       \
      if (lu & flag) {                                                       \
          sb_adds(output, "kernel flag: " #flag "(" txt ")\n");              \
            ...
            CHECK_KERNEL_FLAG(
                PF_FREEZE, "this task is being frozen for suspend now"
            );
            CHECK_KERNEL_FLAG(
            ...
            CHECK_KERNEL_FLAG(
                PF_SPREAD_PAGE, "Spread page cache over cpuset"
            );
            CHECK_KERNEL_FLAG(
                PF_SPREAD_SLAB, "Spread some slab caches over cpuset"
            );
...
#  undef CHECK_KERNEL_FLAG
        ...
        case 9: /*minflt %lu
            ...
            if (lu) {
                sb_addf(
                    output, "minor faults the process has made: %lu\n", lu
                );
            }
            break;
        case 10: /*cminflt %lu
            ...
            if (lu) {
                sb_addf(
                    output,
                    "minor faults that the process's waited-for children "
                    "have made: %lu\n",
                    lu
                );
            }
        ...
        case 11: /*majflt %lu
            ...
            if (lu) {
                sb_addf(
                    output, "major faults the process has made: %lu\n", lu
                );
            }
            break;
        case 12: /*cmajflt %lu
            ...
            if (lu) {
                sb_addf(
                    output,
                    "major faults that the process's waited-for children "
                    "have made: %lu\n",
                    lu
                );
            }
        ...
        case 15: /*cutime %ld
            ...
            if (l) {
                ...
                sb_addf(
                    output,
                    "this process's waited-for children user mode: %dms\n", ms
                );
            }
            break;
        case 16: /*cstime %ld
            ...
            if (l) {
                ...
                sb_addf(
                    output,
                    "this process's waited-for children kernel mode: %dms\n",
                    ms
                );
            }
            break;
        ...
        case 18: /*nice %ld
            ...
            sb_addf(
                output,
                "nice: %ld (from 19 (nicest) to -19 (not nice to others))\n",
                l
            );
            break;
...
static int psinfo_read_maps(pid_t pid, sb_t *output, sb_t *buf)
{
    ...
    while (p && *p) {
        ...
        if (*q != '-') {
            sb_addf(
                output, "Could not parse start: %*pM\n", (int)(end - p), p
            );
            break;
        }
        q++;
        ...
        if (*q != ' ') {
            sb_addf(
                output, "Could not parse stop: %*pM\n", (int)(end - p), p
            );
            break;
        }
...
static FILE *pidproc_open(pid_t pid, const char *what)
{
    char path[PATH_MAX];
    snprintf(
        path, sizeof(path), "/proc/%ld/%s", (long)pid ?: (long)getpid(), what
    );
    return fopen(path, "r");
}
...
void close_fds(int fd_min, qv_t(u32) *nullable to_keep)
{
    ...
    while ((entry = readdir(proc_fds)) != NULL) {
        ...
        if (fd > fd_min &&
            (!to_keep || !contains32(fd, to_keep->tab, to_keep->len)))
        {
            p_close(&fd);
        }
    }
...
int cgroups_get_cpu_count_from_cpuset(void)
{
    ...
    if (lstr_init_from_file(
            &content, CGROUPS_V2_CPUSET_EFFECTIVE_PATH, PROT_READ, MAP_SHARED
        ) < 0 &&
        lstr_init_from_file(
            &content, CGROUPS_V2_CPUSET_PATH, PROT_READ, MAP_SHARED
        ) < 0)
    {
...
int cgroups_get_cpu_quota(int64_t *quota, int64_t *period)
{
    ...
    if (lstr_init_from_file(
            &content, CGROUPS_V2_CPU_MAX_PATH, PROT_READ, MAP_SHARED
        ) < 0)
    {
        return errno == ENOENT ? 0 : -1;
    }
