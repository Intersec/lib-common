...
static inline size_t bsr8(uint8_t u)
{
    return __firstbit_rev8[u];
}
static inline size_t bsr16(uint16_t u)
{
    return __builtin_clz(u) ^ 31;
}
static inline size_t bsr32(uint32_t u)
{
    return __builtin_clz(u) ^ 31;
}
static inline size_t bsr64(uint64_t u)
{
    return __builtin_clzll(u) ^ 63;
}
static inline size_t bsrsz(size_t u)
{
    return DO_SZ(bsr)(u);
}
 ...
 */
ssize_t
bsr(const void *nonnull data, size_t start_bit, size_t len, bool reverse);
...
static inline size_t bsf8(uint8_t u)
{
    return __firstbit_fwd8[u];
}
static inline size_t bsf16(uint16_t u)
{
    return __builtin_ctz(u);
}
static inline size_t bsf32(uint32_t u)
{
    return __builtin_ctz(u);
}
static inline size_t bsf64(uint64_t u)
{
    return __builtin_ctzll(u);
}
static inline size_t bsfsz(size_t u)
{
    return DO_SZ(bsf)(u);
}
 ...
 * \param[in] reverse   if reverse, look for the first 0 instead of the first
 * 1
 ...
 */
ssize_t
bsf(const void *nonnull data, size_t start_bit, size_t len, bool reverse);
...
#  define sign_extend(x, bits)                                               \
      ({                                                                     \
        ...
        __builtin_choose_expr(                                               \
            bitsizeof(x) <= bitsizeof(uint32_t),                             \
...
static inline uint8_t bitcount32(uint32_t n)
{
    return __bitcount11[(n >> 0) & 0x7ff] + __bitcount11[(n >> 11) & 0x7ff] +
           __bitcount11[(n >> 22) & 0x7ff];
}
static inline uint8_t bitcount64(uint64_t n)
{
    return __bitcount11[(n >> 0) & 0x7ff] + __bitcount11[(n >> 11) & 0x7ff] +
           __bitcount11[(n >> 22) & 0x7ff] + __bitcount11[(n >> 33) & 0x7ff] +
           __bitcount11[(n >> 44) & 0x7ff] + __bitcount11[(n >> 55)];
}
#  endif
