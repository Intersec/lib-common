...
__wah_word_enum_start(wah_word_enum_t *en, const wah_words_t *bucket)
{
    ...
    } else if (bucket->tab[en->pos + 1].count > 0) {
    ...
    case WAH_ENUM_LITERAL:
        if (en->pos == bucket.len) {
            if (en->bucket < en->map->_buckets.len - 1) {
                ...
                en->pos = 0;
            } else if ((en->map->len % WAH_BIT_IN_WORD)) {
                en->state = WAH_ENUM_PENDING;
                en->remain_words = 1;
                en->current = en->map->_pending;
                en->current ^= en->reverse;
...
bool wah_bit_enum_scan_word(wah_bit_enum_t *en)
{
    ...
    assert(
        en->word_en.state <= WAH_ENUM_PENDING ||
...
wah_t *wah_init(wah_t *map)
{
     ...
     */
    STATIC_ASSERT(
        offsetof(wah_bucket_t, inlined.len) == offsetof(wah_bucket_t, qv.size)
    );
...
wah_bucket_append(wah_t *map, wah_bucket_t *bucket, wah_word_t word)
{
    if (wah_bucket_is_inlined(bucket)) {
        ...
        wah_bucket_to_qv(
            map->_buckets.mp, bucket, WAH_BUCKET_INLINED_WORDS + 1
        );
    }
 ...
 * storage.
 */
static void wah_bucket_extend(
    wah_t *map, wah_bucket_t *bucket, const wah_word_t *src, int len
)
{
    if (wah_bucket_is_inlined(bucket)) {
 ...
 */
static ALWAYS_INLINE wah_word_t *
wah_bucket_word_ptr(wah_bucket_t *bucket, int pos)
{
    if (wah_bucket_is_inlined(bucket)) {
 ...
 */
static ALWAYS_INLINE wah_header_t *
wah_last_run_header(const wah_t *map, wah_bucket_t *bucket)
{
 ...
 */
static ALWAYS_INLINE uint32_t *wah_last_run_count(const wah_t *map)
{
 ...
 * calling this.
 */
static ALWAYS_INLINE void wah_append_header(wah_t *map, wah_header_t head)
{
    wah_word_t word;
    wah_bucket_t *bucket = tab_last(&map->_buckets);
 ...
 */
static ALWAYS_INLINE void wah_append_literal(wah_t *map, uint32_t val)
{
    wah_word_t word;
    wah_bucket_t *bucket = tab_last(&map->_buckets);
 ...
 */
static void wah_check_normalized(const wah_t *map)
{
 ...
 */
static ALWAYS_INLINE void wah_check_invariant(const wah_t *map)
{
    ...
    assert(
        (int)*wah_last_run_count(map) + map->last_run_pos + 2 ==
 ...
 */
static inline void wah_flatten_last_run(wah_t *map)
{
    wah_bucket_t *bucket = tab_last(&map->_buckets);
    wah_header_t *head;
...
static void wah_push_pending(wah_t *map, uint64_t words, uint64_t active)
{
    ...
    while (words) {
        ...
        to_add =
            MIN(words, (WAH_BITS_IN_BUCKET - bucket_len) / WAH_BIT_IN_WORD);
 ...
 */
static const void *
wah_add_unaligned(wah_t *map, const uint8_t *src, uint64_t count)
{
    while (count >= 64) {
 ...
 */
static void wah_add_aligned(wah_t *map, const uint8_t *src, uint64_t count)
{
 ...
 */
static void
wah_copy_run(wah_t *map, wah_word_enum_t *run, wah_word_enum_t *data)
{
...
void wah_and_(wah_t *map, const wah_t *other, bool map_not, bool other_not)
{
    ...
    while (src_en.state != WAH_ENUM_END || other_en.state != WAH_ENUM_END) {
        ...
        } else if (other_en.state == WAH_ENUM_END) {
...
static void wah_add_en(wah_t *dest, wah_word_enum_t *en, uint64_t words)
{
    ...
    while (en->state != WAH_ENUM_END && words > 0) {
        ...
        case WAH_ENUM_PENDING:
            wah_add_aligned(
                dest, (const uint8_t *)&en->current, WAH_BIT_IN_WORD
            );
            break;
...
#define CONSUME_ALL(amount, skip_first)                                      \
    do {                                                                     \
    ...
    while (enums.len) {
        ...
        tab_for_each_entry(e, &enums) {
            ...
            if (w > first_weight || !first) {
                ...
                first = e;
                first_weight = w;
            } else if (w > second_weight || !second) {
                second = e;
                second_weight = w;
            }
        }
        ...
        if (second && second->state == WAH_ENUM_RUN && !second->current) {
            ...
            continue;
        } else if (enums.len == 1 && first->state != WAH_ENUM_PENDING) {
            ...
            continue;
        } else if (first->state == WAH_ENUM_RUN) {
            if (first->current) {
        ...
        while (buf_pos < end_pos) {
            ...
            case FLAG_LITTERAL:
                if (32 * (end - buf_pos) > bits) {
                    wah_add_aligned(
                        dest, (const uint8_t *)&buffer[buf_pos], bits
                    );
                } else {
                    wah_add_literal(
                        dest, (const uint8_t *)&buffer[buf_pos],
                        4 * (end - buf_pos)
                    );
                }
                break;
...
from_data_split_chunk(from_data_ctx_t *ctx, wah_header_t head, uint64_t words)
{
    ...
    for (;;) {
        ...
        avail_words =
            (WAH_BITS_IN_BUCKET - ctx->bucket_bits_len) / WAH_BIT_IN_WORD;
    ...
    for (;;) {
        ...
        if (words) {
            ...
            map->last_run_pos = 0;
            wah_bucket_append(
                map, ctx->bucket, (wah_word_t){.head = {.words = 0}}
            );
        } else {
...
wah_t *wah_init_from_data(wah_t *map, pstream_t data)
{
    ...
    while (ctx.pos < ctx.size - 1) {
        ...
        if (words) {
            active += membitcount(
                &ctx.tab[ctx.pos + 2], words * sizeof(wah_word_t)
            );
        }
        ...
        if (!overfill_bucket &&
            unlikely(ctx.bucket_bits_len + chunk_len > WAH_BITS_IN_BUCKET))
        {
            /* This wah does not respect the max length of the buckets. */
            e_warning(
                "WAH bucket len is exceeding expected len: %ju > %ju",
            ...
            if (unlikely(ctx.bucket)) {
                ...
                wah_bucket_extend(
                    map, ctx.bucket, (wah_word_t *)head, words + 2
                );
            } else {
...
lstr_t mp_wah_get_storage_lstr(mem_pool_t *mp, const wah_t *wah)
{
    ...
    return LSTR_DATA_V(
        all_buckets.tab, all_buckets.len * sizeof(all_buckets.tab[0])
    );
}
...
static uint64_t wah_debug_print_run(uint64_t pos, const wah_header_t head)
{
    if (head.words != 0) {
        fprintf(
            stderr, "\e[1;30m[%08x] \e[33mRUN %d \e[0m%u words (%llu bits)\n",
            (uint32_t)pos, head.bit, head.words, (uint64_t)head.words * 32ull
        );
    }
    return head.words * 32;
}

static void wah_debug_print_literal(uint64_t pos, const uint32_t lit)
{
    fprintf(
        stderr, "\e[1;30m[%08x] \e[33mLITERAL \e[0m%08x\n", (uint32_t)pos, lit
    );
}

static uint64_t wah_debug_print_literals(uint64_t pos, uint32_t len)
{
    if (len != 0) {
        fprintf(
            stderr, "\e[1;30m[%08x] \e[33mLITERAL \e[0m%u words\n",
            (uint32_t)pos, len
        );
    }
    return len * 32;
...
static void
wah_debug_print_pending(uint64_t pos, const uint32_t pending, int len)
{
    if (len > 0) {
        fprintf(
            stderr, "\e[1;30m[%08x] \e[33mPENDING \e[0m%d bits: %08x\n",
...
void wah_debug_print(const wah_t *wah, bool print_content)
{
    ...
    for (;;) {
        ...
        if (off < bucket_words.len) {
            ...
            fprintf(
                stderr, "  \e[1;32m         CHANGE TO BUCKET %d\e[0m\n",
                bucket_pos + 1
            );
            bucket_words =
                wah_bucket_get_words(&wah->_buckets.tab[bucket_pos]);
            off = 0;
            len = 0;
        }
    }
