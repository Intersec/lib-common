...
#  define MOVED_TO_NEW_FLAT(Path, Count)                                     \
      if ((Path)->hat->do_stats) {                                           \
          ...
          __hat->root->zero_stored_count +=                                  \
              __hat->desc->leaves_per_flat - __moved;                        \
      }
...
static uint32_t qhat_compact_lookup(
    const qhat_compacthdr_t *header, uint32_t from, uint32_t key
)
{
...
static uint32_t
qhat_depth_prefix(const qhat_t *hat, uint32_t key, uint32_t depth)
{
...
static uint32_t
qhat_get_key_bits(const qhat_t *hat, uint32_t key, uint32_t depth)
{
    if (depth == QHAT_DEPTH_MAX) {
...
#define CRITICAL(cond, fmt, ...)                                             \
    do {                                                                     \
        ...
        if (unlikely(!__cond)) {                                             \
            SB_1k(critical_err);                                             \
            debug_stack_scope(                                               \
                DATA_PTR(&critical_err), &txt_debug_ctx_print                \
            );                                                               \
                                                                             \
            sb_setf(                                                         \
                &critical_err,                                               \
                "critical error: condition " #cond " unmet, " fmt,           \
                ##__VA_ARGS__                                                \
            );                                                               \
            ...
            logger_panic(                                                    \
                &hat->qps->logger, "critical anomaly found, aborting"        \
            );                                                               \
        }                                                                    \
    } while (0)
...
static __must_check__ int qhat_flat_check_consistency(
    qhat_t *hat, uint32_t from, uint32_t to, qhat_node_memory_t memory,
    bool *is_suboptimal
)
{
    bool non_null = false;
...
static __must_check__ int qhat_compact_check_consistency(
    qhat_t *hat, uint32_t from, uint32_t to, qhat_node_memory_t memory,
...
)
{
    ...
    CRITICAL(
        ...
        "compact overflow: %u > %u", memory.compact->count,
        hat->desc->leaves_per_compact
    );
    ...
    for (uint32_t i = 0; i < memory.compact->count; i++) {
        ...
        CRITICAL(
            k > prev_key, "bad key order: [%d]%jx >= [%d]%x", i - 1, prev_key,
            i, k
        );
        CRITICAL(
            from <= k && k <= to, "key [%d]%u out of range [%x, %x]", i, k,
            from, to
        );

        if (flags & QHAT_CHECK_CONTENT) {
#define CASE(Size, Compact, Flat)                                            \
    SUBOPTIMAL(!IS_ZERO(Size, Compact->values[i]));
            QHAT_VALUE_LEN_SWITCH(hat, memory, CASE);
#undef CASE
        }
...
static __must_check__ int qhat_node_check_consistency(
    qhat_t *hat, uint32_t key, uint32_t depth, qhat_node_memory_t memory,
    int c, int flags, bool *nullable is_suboptimal
);
...
static void qhat_node_check_ctx_print(int fd, data_t data)
{
    ...
    dprintf(
        fd, "node [%x -> %x] at depth %u, is_leaf=%d, is_compact=%d",
        ctx->key_from, ctx->key_to, ctx->depth, ctx->is_leaf, ctx->is_compact
    );
}

static __must_check__ int qhat_node_check_child(
    qhat_t *hat, uint32_t key, uint32_t from, uint32_t to, uint32_t depth,
    qhat_node_t node, int flags, bool *nullable is_suboptimal
)
{
    ...
    CRITICAL(
        qps_pg_is_in_range(hat->qps, node.page), "bad page number %x",
        node.page
    );
...
#undef CASE
        ...
        CRITICAL(
            page_sz == exp_size, "bad page size for compact %zu != %zu",
            page_sz, exp_size
        );
        CRITICAL(
            memory.compact->parent_left == from, "%u != %u",
            memory.compact->parent_left, from
        );
        CRITICAL(
            memory.compact->parent_right == to, "%u != %u",
            memory.compact->parent_right, to
        );

        RETHROW(qhat_compact_check_consistency(
            hat, key_from, key_to, memory, flags, is_suboptimal
        ));
    } else if (node.leaf) {
        size_t exp_size;
        ...
        CRITICAL(
            page_sz == exp_size, "bad page size for flat %zu != %zu", page_sz,
            exp_size
        );
        CRITICAL(to == from + 1, "to=%x, from=%x", to, from);
        if (flags & QHAT_CHECK_CONTENT && is_suboptimal) {
            RETHROW(qhat_flat_check_consistency(
                hat, key_from, key_to, memory, is_suboptimal
            ));
        }
    } else {
        ...
        RETHROW(qhat_node_check_consistency(
            hat, key_from, depth + 1, memory, QHAT_COUNT, flags, is_suboptimal
        ));
    }
...
)
{
    ...
    for (int i = 0; i <= c; i++) {
        ...
        if (qhat_node_check_child(
                hat, key, from, i, depth, previous, flags, is_suboptimal
            ) < 0)
        {
            if ((flags & QHAT_CHECK_REPAIR_NODES)) {
                logger_notice(
                    &hat->qps->logger,
                    "removing broken node [%x -> %x] depth=%u", from, i, depth
                );
...
qhat_check_consistency_(qhat_t *hat, int flags, bool *nullable is_suboptimal)
{
    ...
    return qhat_node_check_consistency(
        hat, 0, 0, memory, hat->desc->root_node_count, flags, is_suboptimal
    );
}
...
{
    return qhat_check_consistency_flags(
        hat, QHAT_CHECK_CONTENT, is_suboptimal
    );
}
...
static ALWAYS_INLINE qhat_node_memory_t
qhat_node_w_deref_(qps_t *qps, qhat_node_t node)
{
...
static ALWAYS_INLINE qhat_node_const_memory_t
qhat_node_deref_(qps_t *qps, qhat_node_t node)
{
    return qhat_node_w_deref_(qps, node).cst;
}

static ALWAYS_INLINE qhat_node_memory_t qhat_node_w_deref(qhat_path_t *path)
{
    return qhat_node_w_deref_(path->hat->qps, PATH_NODE(path));
}

static ALWAYS_INLINE qhat_node_const_memory_t
qhat_node_deref(const qhat_path_t *path)
{
    return qhat_node_deref_(path->hat->qps, PATH_NODE(path));
}

static qhat_node_memory_t
qhat_parent_w_deref(qhat_path_t *path, uint32_t *nullable node_count)
{
    if (path->depth == 0) {
...
static bool qhat_node_is_pure(const qhat_path_t *path)
{
    if (PATH_NODE(path).leaf) {
        if (PATH_NODE(path).compact) {
            qhat_node_const_memory_t memory = qhat_node_deref(path);
            return memory.compact->parent_left + 1 ==
                   memory.compact->parent_right;
        }
    }
    return true;
}
...
static bool qhat_node_is_empty(const qhat_path_t *path)
{
...
static uint32_t qhat_node_count(const qhat_path_t *path)
{
...
static ALWAYS_INLINE bool qhat_leaf_is_full(const qhat_path_t *path)
{
...
{
    uint32_t pages =
        compact ? hat->desc->pages_per_compact : hat->desc->pages_per_flat;
    qps_pg_t page;
...
static NEVER_INLINE void qhat_split_leaf(qhat_path_t *path)
{
    qhat_node_t node;
    ...
    if (qhat_node_is_pure(path)) {
        ...
        e_named_trace(
            2, "trie/insert/split",
            "add intermediate node %u above node %u (depth: %d)",
...

    {
        ...
        if (path->depth > 0) {
            prefix = qhat_depth_prefix(path->hat, path->key, path->depth - 1);
        }
        prefix |= qhat_lshift(path->hat, split, path->depth);
        e_named_trace(
            4, "trie/insert/split",
            "key %x, splitting at prefix %x (depth %d, split %x)", path->key,
            prefix, path->depth, split
        );
        ...
        if (sep == 0 || sep == count) {
            ...
            assert(
                compact->parent_left != prev_parent_start ||
            ...
            e_named_trace(
                3, "trie/insert/split",
                "split [%u-%u] at %u (%d elements, depth %d)",
                compact->parent_left, compact->parent_right - 1, split, sep,
                path->depth
            );
...
#undef CASE
                ...
                sep > path->hat->desc->split_compact_threshold &&
                path->depth == QHAT_DEPTH_MAX - 1 &&
...
static void qhat_optimize_parent(qhat_path_t *path)
{
        ...
        path->depth > 0)
    {
        ...
        e_named_trace(
            2, "trie/optimize", "removing dispatch node %u",
            PATH_NODE(path).page
        );
...
#define CASE(Size, Compact, Flat)                                            \
    ...
    p_copy(                                                                  \
        first_compact->keys + first_compact->count, Compact->keys,           \
        Compact->count                                                       \
    );                                                                       \
    p_copy(                                                                  \
        first_compact->values + first_compact->count, Compact->values,       \
        Compact->count                                                       \
    );                                                                       \
...
#undef CASE
    ...
    e_named_trace(
        2, "trie/optimize/merge", "merged leaf %u in %u", second.page,
        PATH_NODE(path).page
    );
    qhat_unmap_node(path->hat, second);
}

static NEVER_INLINE void qhat_optimize(qhat_path_t *path)
{
           ...
           qhat_node_is_empty(path))
    {
        ...
        e_named_trace(
            2, "trie/optimize", "removing empty bucket %u", node.page
        );
...

    {
        ...
        e_named_trace(
            3, "trie/optimize/merge",
            "merging siblings of %u "
...
static void qhat_wipe_dispatch_node(
    qhat_t *hat, qhat_node_const_memory_t memory, size_t max
)
{
...
__attribute__((constructor)) static void qhat_initializes(void)
{
...
void qhat_fix_stored0(qhat_t *hat)
{
    ...
    for (qhat_tree_enumerator_t en = qhat_get_tree_enumerator(hat); !en.end;
         qhat_tree_enumerator_next(&en, true))
    {
        if (en.compact) {
...
static void
qhat_tree_enumerator_find_entry_from(qhat_tree_enumerator_t *en, uint32_t key)
{
    if (en->compact) {
 ...
 */
static void
qhat_tree_enumerator_find_forward(qhat_tree_enumerator_t *en, uint32_t key)
{
    qhat_t *hat = en->path.hat;
    ...
    if (key < last_key) {
        qhat_tree_enumerator_find_entry_from(en, key);
    } else if (qhat_get_key_bits(hat, diff, 0)) {
        qhat_tree_enumerator_find(en, key);
    } else if (en->path.depth >= 1 && qhat_get_key_bits(hat, diff, 1)) {
        en->path.depth = 0;
        qhat_tree_enumerator_find_node(en, key);
    } else if (en->path.depth >= 2 && qhat_get_key_bits(hat, diff, 2)) {
        en->path.depth = 1;
        qhat_tree_enumerator_find_node(en, key);
    } else {
        qhat_tree_enumerator_find_entry_from(en, key);
...
void qhat_enumerator_next(qhat_enumerator_t *en, bool safe)
{
    if (en->is_nullable) {
        ...
        qhat_tree_enumerator_next(&en->nn_trie, safe);
    }
}
...
qhat_enumerator_t qhat_get_enumerator_at(qhat_t *trie, uint32_t key)
{
    ...
    if (trie->root->is_nullable) {
        ...
        en.nu_trie = qhat_get_tree_enumerator_at(trie, key);
        en.is_nullable = true;
    } else {
        en.nn_trie = qhat_get_tree_enumerator_at(trie, key);
    }
    return en;
}
...
qhat_enumerator_t qhat_get_enumerator(qhat_t *trie)
{
    ...
    /* 'key' field. */
    STATIC_ASSERT(
        offsetof(qhat_enumerator_t, key) ==
        offsetof(qhat_enumerator_t, nn_trie.key)
    );
    STATIC_ASSERT(
        offsetof(qhat_enumerator_t, key) ==
    ...
    /* 'end' field. */
    STATIC_ASSERT(
        offsetof(qhat_enumerator_t, end) ==
        offsetof(qhat_enumerator_t, nn_trie.end)
    );
    STATIC_ASSERT(
        offsetof(qhat_enumerator_t, end) ==
    ...
    /* 'is_nullable' field. */
    STATIC_ASSERT(
        offsetof(qhat_enumerator_t, is_nullable) ==
        offsetof(qhat_enumerator_t, nn_trie.is_nullable)
    );
    STATIC_ASSERT(
        offsetof(qhat_enumerator_t, is_nullable) ==
...
void qhat_enumerator_go_to(qhat_enumerator_t *en, uint32_t key, bool safe)
{
    if (en->is_nullable) {
        ...
        qhat_tree_enumerator_go_to(&en->nn_trie, key, safe);
    }
}
...
qhat_path_t qhat_enumerator_get_path(const qhat_enumerator_t *en)
{
    ...
    tree_en = en->is_nullable ? &en->nu_trie : &en->nn_trie;
 ...
 */
static void
qhat_tree_enumerator_enter_leaf(qhat_tree_enumerator_t *en, uint32_t key)
{
...
void qhat_tree_enumerator_find(qhat_tree_enumerator_t *en, uint32_t key)
{
    ...
    i = scan_non_zero32(
        &hat->root->nodes[0].value, root, hat->desc->root_node_count
    );
    if (i >= 0) {
...
void qhat_tree_enumerator_find_node(qhat_tree_enumerator_t *en, uint32_t key)
{
    ...
    if ((i = scan_non_zero32(&memory.nodes[0].value, pos, QHAT_COUNT)) >= 0) {
        ...
        en->path.key =
            qhat_depth_prefix(hat, en->path.key, en->path.depth - 1);
...
__attr_flatten__ qhat_tree_enumerator_t
qhat_get_tree_enumerator_at(qhat_t *trie, uint32_t key)
{
...
#define QHAT_DISSECT_NODE(Cond, Adr_inside_qps, Fmt, ...)                    \
    do {                                                                     \
        ...
        if (unlikely(!__cond)) {                                             \
            ...
            qps_dissect_notify_qhat_err(                                     \
                ctx, QPS_ANOMALY_QHAT_ISSUE, Adr_inside_qps                  \
            );                                                               \
            return;                                                          \
        }                                                                    \
    } while (0)

#define QHAT_DISSECT_SUBOPTIMAL(Cond, Adr_inside_qps)                        \
    do {                                                                     \
        bool __cond = (Cond);                                                \
                                                                             \
        if (unlikely(!__cond)) {                                             \
            qps_dissect_notify_qhat_err(                                     \
                ctx, QPS_ANOMALY_QHAT_NODE_SUBOPTIMAL, Adr_inside_qps        \
            );                                                               \
        }                                                                    \
    } while (0)

static void qhat_dissect_flat_node(
    qps_dissect_ctx_t *ctx, qhat_t *hat, uint32_t from, uint32_t to,
    qhat_node_memory_t memory
)
{
    bool non_null = false;
...
#undef CASE
    ...
    QHAT_DISSECT_SUBOPTIMAL(non_null, memory.raw);
}

static void qhat_dissect_compact_node(
    qps_dissect_ctx_t *ctx, qhat_t *hat, uint32_t from, uint32_t to,
    qhat_node_memory_t memory
)
{
    ...
    QHAT_DISSECT_SUBOPTIMAL(memory.compact->count > 0, memory.raw);
    if (memory.compact->count == 0) {
        return;
    }

    QHAT_DISSECT_NODE(
        memory.compact->count <= hat->desc->leaves_per_compact, memory.raw,
        "compact overflow: %u > %u", memory.compact->count,
        hat->desc->leaves_per_compact
    );
...
#define CASE(Size, Compact, Flat)                                            \
    QHAT_DISSECT_SUBOPTIMAL(                                                 \
        !IS_ZERO(Size, Compact->values[i]), &Compact->values[i]              \
    );
...
static void qhat_dissect_node(
    qps_dissect_ctx_t *ctx, qhat_t *hat, uint32_t key, uint32_t depth,
    qhat_node_memory_t memory, int c
);

static void qhat_dissect_child_node(
    qps_dissect_ctx_t *ctx, qhat_t *hat, uint32_t key, uint32_t from,
...
#undef CASE
        ...
        QHAT_DISSECT_NODE(
            page_sz == exp_size, memory.raw,
            "bad page size "
            "for compact %zu != %zu",
            page_sz, exp_size
        );
        QHAT_DISSECT_NODE(
            memory.compact->parent_left == from, memory.raw, "%u != %u",
            memory.compact->parent_left, from
        );
        QHAT_DISSECT_NODE(
            memory.compact->parent_right == to, memory.raw, "%u != %u",
            memory.compact->parent_right, to
        );
        ...
        QHAT_DISSECT_NODE(
            to == from + 1, memory.raw, "to=%x, from=%x", to, from
        );

        qhat_dissect_flat_node(ctx, hat, key_from, key_to, memory);
    } else {
        QHAT_DISSECT_NODE(
            page_sz == 1, memory.raw,
        ...
        QHAT_DISSECT_NODE(
            depth < QHAT_DEPTH_MAX, memory.raw, "depth=%u", depth
        );
...
void qhat_dissect(qps_dissect_ctx_t *ctx, qhat_t *hat)
{
    ...
    if (memcmp(QPS_TRIE_SIG, hat->root->sig, sizeof(QPS_TRIE_SIG))) {
        ...
        sb_setf(
            ctx->err, "invalid qhat signature found, hex is: \"%*pX\"",
            (int)sizeof(hat->root->sig), hat->root->sig
        );
        qps_dissect_notify_handle_err(
            ctx, QPS_ANOMALY_INVALID_H_REF_CONTENT, &cache->handle,
            cache->data
        );
        return;
    } else if (
        hat->root->value_len > 16 || bitcount32(hat->root->value_len) != 1
    )
    {
        sb_setf(
            ctx->err, "invalid \"value_len\" field for qhat (%u)",
            hat->root->value_len
        );
        qps_dissect_notify_handle_err(
            ctx, QPS_ANOMALY_INVALID_H_REF_CONTENT, &cache->handle,
...
static void qhat_get_dispatch_nodes(
    qhat_t *hat, qhat_node_const_memory_t mem, size_t max, qps_roots_t *roots
);

static void
qhat_get_qps_nodes(qhat_t *hat, qhat_node_t node, qps_roots_t *roots)
{
    if (!node.value) {
        return;
    }
...
static void qhat_compute_counts_(
    qhat_t *hat, qhat_root_t *root, qhat_node_const_memory_t mem, size_t max
)
{
    qhat_node_t current = QHAT_NULL_NODE;
...
static void qhat_debug_print_node(
    const qhat_t *hat, uint32_t flags, int depth, uint32_t prefix,
    qhat_node_t node, bool pure, FILE *stream
);

static void qhat_debug_print_dispatch_node(
    const qhat_t *hat, uint32_t flags, int depth, uint32_t prefix,
    const qhat_node_t *pointers, int end, FILE *stream
)
{
    ...
    if (value.value != 0) {
        ...
        qhat_debug_print_node(
            hat, flags, depth + 1, prefix | previous, value,
            (end - previous) == 1, stream
        );
    }
}

static void qhat_debug_print_compact_leaf(
    const qhat_t *hat, uint32_t flags, int depth, uint32_t prefix,
    qhat_node_const_memory_t memory, FILE *stream
)
{
...
static ALWAYS_INLINE bool qhat_debug_is_flat_default(
    const qhat_t *hat, qhat_node_const_memory_t memory, uint8_t pos
)
{
...
static void qhat_debug_print_flat_leaf(
    const qhat_t *hat, uint32_t flags, int depth, uint32_t prefix,
    qhat_node_const_memory_t memory, FILE *stream
)
{
...
)
{
    ...
    fprintf(
        stream, "%s node %u: prefix=%x/%x", (pure ? "Pure" : "Hybrid"),
        node.page, prefix, qhat_depth_prefix(hat, 0xffffffffu, depth - 1)
    );
    if (node.leaf) {
        fprintf(stream, ", leaf");
        if (node.compact) {
            memory.raw = qps_pg_deref(hat->qps, node.page);
            fprintf(
                stream, " (compact, %d entries, parent %x -> %x)\n",
                memory.compact->count, memory.compact->parent_left,
                memory.compact->parent_right - 1
            );
            qhat_debug_print_compact_leaf(
                hat, flags, depth, prefix, memory, stream
            );
        } else {
            fprintf(stream, " (flat)\n");
            memory.raw = qps_pg_deref(hat->qps, node.page);
            qhat_debug_print_flat_leaf(
                hat, flags, depth, prefix, memory, stream
            );
        }
    } else {
        fprintf(stream, "\n");
        memory.raw = qps_pg_deref(hat->qps, node.page);
        qhat_debug_print_dispatch_node(
            hat, flags, depth, prefix, memory.nodes, QHAT_COUNT, stream
        );
    }
}
...
void qhat_debug_print_stream(qhat_t *hat, uint32_t flags, FILE *stream)
{
    ...
    qhat_debug_print_dispatch_node(
        hat, flags, 0, 0U, hat->root->nodes, hat->desc->root_node_count,
        stream
    );
    fprintf(stream, "\n");
}
