...
#  define CC_HAS_WARNING(warn_option, gnuc_maj, gnuc_min)                    \
      /* Note: __has_warning exist since clang 3.6 and do not exist at all   \
       * for GCC, which is why checking for GCC version with __GNUC_PREREQ   \
       * is necessary.                                                       \
       */                                                                    \
      (__has_warning(warn_option) || __GNUC_PREREQ(gnuc_maj, gnuc_min))

#  if __GNUC_PREREQ(4, 6) || __CLANG_PREREQ(3, 0)
#    define CC_WARNING_IGNORE_PUSH _Pragma("GCC diagnostic push")
#    define CC_WARNING_IGNORE_POP _Pragma("GCC diagnostic pop")
#  else
...
#    define CC_WARNING_IGNORE_SIGN_COMPARE CC_WARNING_IGNORE("-Wsign-compare")
#  else
...
#    define CC_WARNING_IGNORE_UNUSED_VALUE CC_WARNING_IGNORE("-Wunused-value")
#  else
#    define CC_WARNING_IGNORE_UNUSED_VALUE
#  endif

#  if CC_HAS_WARNING("-Warray-bounds", 14, 0)
#    define CC_WARNING_IGNORE_ARRAY_BOUNDS CC_WARNING_IGNORE("-Warray-bounds")
#  else
...
#      define __must_be_array(a)                                             \
          (sizeof(char                                                       \
                      [1 - 2 * __builtin_types_compatible_p(                 \
                                   typeof(a), typeof(&(a)[0])                \
                               )]) -                                         \
           1)
#    endif
...
#    define __attr_printf__(a, b)                                            \
        __attribute__((format(printf, a, b), __nonnull__(a)))
#    define __attr_scanf__(a, b)                                             \
        __attribute__((format(scanf, a, b), __nonnull__(a)))
...
#        define ALWAYS_INLINE                                                \
            inline __attribute__((always_inline, artificial))
#      else
 ...
 * \param[in]  expr    the expression you want to be always true at compile *
 * time.
 * \safemacro
 *
 */
...
#      define STATIC_ASSERT(cond)                                            \
          __builtin_choose_expr(                                             \
              cond, (void)0, __error__("static assertion failed: " #cond "") \
          )
#      define ASSERT_COMPATIBLE(e1, e2)                                      \
          STATIC_ASSERT(__builtin_types_compatible_p(typeof(e1), typeof(e2)))
#    else
...
#  define countof(table)                                                     \
      (cast(                                                                 \
          ssize_t,                                                           \
          sizeof(table) / sizeof((table)[0]) + __must_be_array(table)        \
      ))
...
#  define BITMASK_NTH(type_t, n)                                             \
      (cast(type_t, 1) << ((n) & (bitsizeof(type_t) - 1)))
#  define BITMASK_LT(type_t, n) (BITMASK_NTH(type_t, n) - 1)
#  define BITMASK_LE(type_t, n) ((BITMASK_NTH(type_t, n) << 1) - 1)
#  define BITMASK_GE(type_t, n)                                              \
      (~cast(type_t, 0) << ((n) & (bitsizeof(type_t) - 1)))
...
#  define FOR_INSTR(ctx, instr) for (instr; !ctx##stop; ctx##stop = true)
...
#  define _tab_for_each_ptr(ptr_var, ptr_cpy_var, vec_var, _vec)             \
      FOR_INSTR1(                                                            \
          _tab_for_each_ptr##ptr_var, typeof(*_vec) *vec_var = (_vec)        \
      )                                                                      \
      for (__attr_unused__ typeof(*vec_var->tab) *                           \
               ptr_var = vec_var->tab,                                       \
...
#  define tab_for_each_ptr(ptr_var, _vec)                                    \
      _tab_for_each_ptr(                                                     \
          ptr_var, tab_for_each_ptr_ptr_cpy##ptr_var,                        \
          tab_for_each_ptr_vec##ptr_var, (_vec)                              \
      )

#  define _tab_enumerate_ptr(pos_var, ptr_var, vec_var, _vec)                \
      FOR_INSTR2(                                                            \
          _tab_enumerate_ptr##ptr_var, typeof(*_vec) *vec_var = (_vec),      \
...
#  define tab_enumerate_ptr(pos_var, ptr_var, _vec)                          \
      _tab_enumerate_ptr(                                                    \
          pos_var, ptr_var, tab_enumerate_ptr_vec##ptr_var, (_vec)           \
      )

#  define _tab_enumerate(pos_var, entry_var, vec_var, _vec)                  \
      FOR_INSTR2(                                                            \
          _tab_enumerate##entry_var, typeof(*_vec) *vec_var = (_vec),        \
          __attr_unused__ typeof(*vec_var->tab) entry_var                    \
      )                                                                      \
      for (int pos_var = 0; pos_var < vec_var->len &&                        \
...
#  define tab_for_each_pos_safe(pos, vec) tab_for_each_pos_rev(pos, vec)
...
#  define carray_for_each_ptr(ptr, array)                                    \
      for (__attr_unused__ typeof(*array) *ptr = (array),                    \
                                          *__i_##ptr = (array);              \
           __i_##ptr < (array) + countof(array); ptr = ++__i_##ptr)
...
#    define _defer_with_name(_code, _dname)                                  \
        __attr_cleanup__(_defer_blk_cleanup)                                 \
            __attr_unused__ void (^_dname)(void) = ^(void) {                 \
            _code                                                            \
            }

#    define defer(_code) _defer_with_name(_code, PFX_LINE(__defer__))
...
#    define defer(_code)                                                     \
        _defer_with_name(                                                    \
            _code, PFX_LINE_SFX(__defer__, _func),                           \
