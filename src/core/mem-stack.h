 ...
 * allocating blocks of sizes < 2, < 4, < 8, < 16 or >= 16 yield blocks
 * aligned to a 1, 2, 4, 8, and 16 boundary respectively.
 *
...
#  ifdef MEM_BENCH
    ...
    struct mem_bench_t *nonnull mem_bench;
#  endif
} mem_stack_pool_t;

mem_stack_pool_t *nonnull mem_stack_pool_init(
    mem_stack_pool_t *nonnull, const char *nonnull name, int initialsize,
    unsigned flags
) __attr_leaf__;
...
mem_pool_t *nonnull mem_stack_new_flags(
    const char *nonnull name, int initialsize, unsigned flags
);
void mem_stack_delete(mem_pool_t * nonnull * nullable mp);

static inline mem_stack_pool_t *nonnull
mem_stack_get_pool(mem_pool_t *nonnull mp)
{
...
void mem_stack_pool_protect(
    mem_stack_pool_t *nonnull sp, const mem_stack_frame_t *nonnull up_to
);
/*
...
#  else
#    define mem_stack_pool_prev(frame) ((mem_stack_frame_t *)(frame)->prev)
...
mem_stack_pool_is_at_top(const mem_stack_pool_t *nonnull sp)
{
    return sp->stack == &sp->base;
}

const void *nonnull
mem_stack_pool_push(mem_stack_pool_t *nonnull) __attr_leaf__;
#  ifndef NDEBUG
const void *nonnull mem_stack_pool_pop_libc(mem_stack_pool_t *nonnull);
#  endif

#  ifdef MEM_BENCH
void mem_stack_pool_bench_pop(
    mem_stack_pool_t *nonnull, mem_stack_frame_t *nonnull
);
#  endif
...
static ALWAYS_INLINE const void *nonnull
mem_stack_pool_pop(mem_stack_pool_t *nonnull sp)
{
...
#  define ta_realloc_extra_from(pp, old_extra, new_extra, alignment)         \
      mpa_realloc_extra_from(                                                \
          t_pool(), (pp), (old_extra), (new_extra), (alignment)              \
      )
...
#  define ta_realloc_extra_field_from(                                       \
      pp, field, old_count, new_count, alignment                             \
  )                                                                          \
      mpa_realloc_extra_field_from(                                          \
          t_pool(), (pp), field, (old_count), (new_count), (alignment)       \
      )
#  define ta_realloc0_extra_field(                                           \
      pp, field, old_count, new_count, alignment                             \
  )                                                                          \
 ...
 *     }
 */
static ALWAYS_INLINE void t_scope_cleanup(const void *nonnull *nonnull unused)
{
#    ifndef NDEBUG
...
#    define t_scope TScope PFX_LINE(t_scope_because_cpp_sucks_donkeys_)
#  endif
