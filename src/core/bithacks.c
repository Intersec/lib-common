...
#define SCAN_WORD(Low, Up)                                                   \
    do {                                                                     \
        uint64_t w = reverse ? ~*pos : *pos;                                 \
                                                                             \
        if (Low)                                                             \
            w &= BITMASK_GE(uint64_t, Low);                                  \
        if (Up != 64)                                                        \
            w &= BITMASK_LT(uint64_t, Up);                                   \
                                                                             \
        if (w) {                                                             \
...
size_t membitcount_c(const void *ptr, size_t n)
{
    ...
    if (unlikely((uintptr_t)ptr & 3)) {
        ...
        c1 += bitcount32(cpu_to_le32(p32[-1]) & BITMASK_GE(uint32_t, 8 * dt));
    }
    for (; i + 8 <= n / 4; i += 8) {
...
__attr_noasan__ __attribute__((target("popcnt")))
...
#  else
        ...
        if (n % 4) {
            /* if valgrinds cringes here, tell hum to STFU */
            c1 +=
                __builtin_popcount(p[i] & BITMASK_LT(uint32_t, 8 * (n % 4)));
        }
#  endif
        return c1 + c2 + c3 + c4;
    }
...
__attr_noasan__ __attribute__((target("ssse3")))
