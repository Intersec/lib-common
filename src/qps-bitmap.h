...
qps_bitmap_state_t
qps_bitmap_get(qps_bitmap_t *map, uint32_t row) __attr_leaf__;
qps_bitmap_state_t
qps_bitmap_set(qps_bitmap_t *map, uint32_t row) __attr_leaf__;
qps_bitmap_state_t
qps_bitmap_reset(qps_bitmap_t *map, uint32_t row) __attr_leaf__;
qps_bitmap_state_t
qps_bitmap_remove(qps_bitmap_t *map, uint32_t row) __attr_leaf__;

void qps_bitmap_compute_stats(
    qps_bitmap_t *map, size_t *memory, uint32_t *entries, uint32_t *slots
) __attr_leaf__;
...
static inline void qps_bitmap_enumerator_find_dispatch_nu(
    qps_bitmap_enumerator_t *en, qps_bitmap_key_t key
);

static inline void qps_bitmap_enumerator_find_leaf_nu(
    qps_bitmap_enumerator_t *en, qps_bitmap_key_t key
);
static inline void qps_bitmap_enumerator_find_bit_nu(
    qps_bitmap_enumerator_t *en, qps_bitmap_key_t key
);
static inline void qps_bitmap_enumerator_find_word_nu(
    qps_bitmap_enumerator_t *en, qps_bitmap_key_t key
);

static inline void qps_bitmap_enumerator_dispatch_up_nu(
    qps_bitmap_enumerator_t *en, qps_bitmap_key_t key,
...
)
{
    ...
    } else if (key.dispatch != new_key.dispatch) {
...
)
{
    ...
    for (unsigned i = key.root; i < QPS_BITMAP_ROOTS; i++) {
        if (en->map->root->roots[i] != 0) {
            en->key.key = 0;
            en->key.root = i;
            en->dispatch = (const qps_bitmap_dispatch_t *)qps_pg_deref(
                en->map->qps, en->map->root->roots[i]
            );
            if (key.root != i) {
                key = en->key;
            }
...
)
{
    ...
    for (unsigned i = key.dispatch; i < QPS_BITMAP_DISPATCH; i++) {
        if ((*en->dispatch)[i].node != 0) {
            ...
            en->key.bit = 0;
            en->key.dispatch = i;
            en->leaf = (const uint64_t *)qps_pg_deref(
                en->map->qps, (*en->dispatch)[i].node
            );
            if (key.dispatch != i) {
                key = en->key;
            }
...
static inline void
qps_bitmap_enumerator_next_nu(qps_bitmap_enumerator_t *en, bool safe)
{
...
static inline void qps_bitmap_enumerator_go_to_nu(
    qps_bitmap_enumerator_t *en, uint32_t row, bool safe
)
{
    qps_bitmap_key_t key;
    ...
    if (en->key.root < key.root) {
        qps_bitmap_enumerator_find_dispatch_nu(en, key);
    } else if (en->key.dispatch < key.dispatch) {
        qps_bitmap_enumerator_find_leaf_nu(en, key);
    } else if (en->key.word_null < key.word_null) {
        qps_bitmap_enumerator_find_word_nu(en, key);
    } else {
        qps_bitmap_enumerator_find_bit_nu(en, key);
    }
...
static inline void qps_bitmap_enumerator_find_bit_nn(
    qps_bitmap_enumerator_t *en, qps_bitmap_key_t key
);
static inline void qps_bitmap_enumerator_find_leaf_nn(
    qps_bitmap_enumerator_t *en, qps_bitmap_key_t key
);
static inline void qps_bitmap_enumerator_find_word_nn(
    qps_bitmap_enumerator_t *en, qps_bitmap_key_t key
);

static inline void qps_bitmap_enumerator_find_dispatch_nn(
    qps_bitmap_enumerator_t *en, qps_bitmap_key_t key
);

static inline void qps_bitmap_enumerator_dispatch_up_nn(
    qps_bitmap_enumerator_t *en, qps_bitmap_key_t key,
    qps_bitmap_key_t new_key
)
{
...
static inline void
qps_bitmap_enumerator_next_nn(qps_bitmap_enumerator_t *en, bool safe)
{
    qps_bitmap_key_t key = en->key;
...
static inline void qps_bitmap_enumerator_go_to_nn(
    qps_bitmap_enumerator_t *en, uint32_t row, bool safe
)
{
    qps_bitmap_key_t key;
...
static inline qps_bitmap_enumerator_t
qps_bitmap_get_enumerator_at(qps_bitmap_t *map, uint32_t row)
{
...
static inline qps_bitmap_enumerator_t
qps_bitmap_get_enumerator(qps_bitmap_t *map)
{
    return qps_bitmap_get_enumerator_at(map, 0);
}

static inline void
qps_bitmap_enumerator_next(qps_bitmap_enumerator_t *en, bool safe)
{
    if (en->is_nullable) {
...
static inline void qps_bitmap_enumerator_go_to(
    qps_bitmap_enumerator_t *en, uint32_t row, bool safe
)
{
    if (en->is_nullable) {
...
void qps_bitmap_get_qps_roots(
    qps_bitmap_t *map, qps_roots_t *roots
) __attr_leaf__;
