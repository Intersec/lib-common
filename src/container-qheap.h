...
#define qhp_funcs_t(n, type_t, is_min_heap, cmp, set_pos)                    \
                                                                             \
    __attr_unused__ static ALWAYS_INLINE int __qhp_##n##_set(                \
        qhp_t(n) *heap, int pos, type_t node                                 \
    )                                                                        \
    {                                                                        \
        set_pos(node, pos);                                                  \
        heap->tab[pos] = node;                                               \
        ...
        return pos;                                                          \
                                                                             ...
                                                                             \
    __attr_unused__ static inline int __qhp_##n##_down(                      \
        qhp_t(n) *heap, int _pos                                             \
    )                                                                        \
    {                                                                        \
        ...
        while ((child_pos = QHP_CHILD(pos, 0)) + QHP_FANOUT <= heap->len) {  \
            ...
            int n = 0; /* pos of the minimum in c[0 .. QHP_FANOUT] */        \
                                                                             \
            if (QHP_CMP(is_min_heap, cmp, c[1], <, c[n]))                    \
                n = 1;                                                       \
            if (QHP_CMP(is_min_heap, cmp, c[2], <, c[n]))                    \
                n = 2;                                                       \
            if (QHP_CMP(is_min_heap, cmp, c[3], <, c[n]))                    \
                n = 3;                                                       \
        ...
        if (child_pos < heap->len) {                                         \
            ...
            int n = 0; /* pos of the minimum in c[0 .. QHP_FANOUT] */        \
                                                                             \
            if (c + 1 < E && QHP_CMP(is_min_heap, cmp, c[1], <, c[n]))       \
                n = 1;                                                       \
            if (c + 2 < E && QHP_CMP(is_min_heap, cmp, c[2], <, c[n]))       \
                n = 2;                                                       \
            if (c + 3 < E && QHP_CMP(is_min_heap, cmp, c[3], <, c[n]))       \
                n = 3;                                                       \
            STATIC_ASSERT(QHP_FANOUT == 4);                                  \
                                                                             \
            if (QHP_CMP(is_min_heap, cmp, c[n], <, node)) {                  \
        ...
        return _pos != pos ? __qhp_##n##_set(heap, pos, node) : _pos;        \
                                                                             ...
                                                                             \
    __attr_unused__ static inline int __qhp_##n##_up(                        \
        qhp_t(n) *heap, int _pos                                             \
    )                                                                        \
    {                                                                        \
        type_t node = heap->tab[_pos];                                       \
        ...
        return _pos != pos ? __qhp_##n##_set(heap, pos, node) : _pos;        \
                                                                             ...
                                                                             \
    __attr_unused__ static inline int qhp_##n##_insert(                      \
        qhp_t(n) *heap, type_t node                                          \
    )                                                                        \
    {                                                                        \
        ...
        return __qhp_up(n, heap, heap->len - 1);                             \
                                                                             ...
                                                                             \
    __attr_unused__ static inline int qhp_##n##_fixup(                       \
        qhp_t(n) *heap, int pos                                              \
    )                                                                        \
    {                                                                        \
        ...
        if (pos > 0 && QHP_CMP(is_min_heap, cmp, *parent, >=, *node)) {      \
            ...
            return __qhp_down(n, heap, pos);                                 \
                                                                             ...
                                                                             \
    __attr_unused__ static inline type_t qhp_##n##_remove(                   \
        qhp_t(n) *heap, int pos                                              \
    )                                                                        \
    {                                                                        \
        type_t node;                                                         \
