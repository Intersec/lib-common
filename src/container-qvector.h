...
#ifdef __has_blocks
typedef int(BLOCK_CARET qvector_cmp_b)(
    const void *nonnull, const void *nonnull
);
...
static inline qvector_t *nonnull __qvector_init(
    qvector_t *nonnull vec, void *nullable buf, int blen, int bsize,
    mem_pool_t *nullable mp
)
{
    *vec = (qvector_t){
...
void qvector_reset(qvector_t *nonnull vec, size_t v_size) __attr_leaf__;
void qvector_wipe(qvector_t *nonnull vec, size_t v_size) __attr_leaf__;
void __qvector_grow(
    qvector_t *nonnull, size_t v_size, size_t v_align, int extra
) __attr_leaf__;
void __qvector_optimize(
    qvector_t *nonnull, size_t v_size, size_t v_align, size_t size
) __attr_leaf__;
void *nonnull __qvector_splice(
    qvector_t *nonnull, size_t v_size, size_t v_align, int pos, int rm_len,
    int inserted_len
) __attr_leaf__;
#ifdef __has_blocks

void __qv_sort(
    void *nonnull tab, size_t v_size, size_t len, qvector_cmp_b nonnull cmp
);

void __qvector_shuffle(qvector_t *nonnull vec, size_t v_size);
void __qvector_diff(
    const qvector_t *nonnull vec1, const qvector_t *nonnull vec2,
...
int __qvector_bisect(
    const qvector_t *nonnull vec, size_t v_size, const void *nonnull elt,
    bool *nullable found, qvector_cmp_b nonnull cmp
);
int __qvector_find(
    const qvector_t *nonnull vec, size_t v_size, const void *nonnull elt,
    bool sorted, qvector_cmp_b nonnull cmp
);
bool __qvector_contains(
    const qvector_t *nonnull vec, size_t v_size, const void *nonnull elt,
    bool sorted, qvector_cmp_b nonnull cmp
);
void __qvector_uniq(
    qvector_t *nonnull vec, size_t v_size, qvector_cmp_b nonnull cmp,
    qvector_del_b nullable del
);
void __qvector_deep_extend(
    qvector_t *nonnull vec_dst, const qvector_t *nonnull vec_src,
    size_t v_size, size_t v_align, qvector_cpy_b nonnull cpy_f
);
#endif
 ...
 */
static inline void qvector_optimize(
    qvector_t *nonnull vec, size_t v_size, size_t v_align, size_t ratio,
    size_t extra_ratio
)
{
    ...
    if (vec->len * ratio < 100 * cur_waste) {
        __qvector_optimize(
            vec, v_size, v_align, vec->len + vec->len * extra_ratio / 100
        );
    }
}

static inline void *nonnull
qvector_grow(qvector_t *nonnull vec, size_t v_size, size_t v_align, int extra)
{
...
static inline void *nonnull qvector_growlen(
    qvector_t *nonnull vec, size_t v_size, size_t v_align, int extra
)
{
    void *res;
...
uint64_t __qvector_grow_get_new_alloc_size(
    qvector_t *nonnull vec, size_t v_size, int extra
);

static inline void *nonnull qvector_splice(
    qvector_t *nonnull vec, size_t v_size, size_t v_align, int pos,
    int rm_len, const void *nullable inserted_values, int inserted_len
)
{
    void *res;
    ...
    if (__builtin_constant_p(inserted_len)) {
        ...
        {
            memmove(
                vec->tab + v_size * (pos + inserted_len),
    ...
    if (__builtin_constant_p(rm_len) && rm_len == 0 && pos == vec->len) {
        ...
        res =
            __qvector_splice(vec, v_size, v_align, pos, rm_len, inserted_len);
    }
...
#  define __QVECTOR_BASE_BLOCKS(pfx, cval_t, val_t)                          \
      ...
      typedef void(BLOCK_CARET pfx##_cpy_b)(                                 \
          val_t * nonnull a, cval_t * nonnull b                              \
      );                                                                     \
                                                                             \
      __attr_unused__ static inline void pfx##_sort(                         \
          pfx##_t *nonnull vec, pfx##_cmp_b nonnull cmp                      \
      )                                                                      \
      {                                                                      \
          __qv_sort(                                                         \
              vec->qv.tab, sizeof(val_t), vec->qv.len, (qvector_cmp_b)cmp    \
          );                                                                 \
      }                                                                      \
      __attr_unused__ static inline void pfx##_diff(                         \
          const pfx##_t *nonnull vec1, const pfx##_t *nonnull vec2,          \
          pfx##_t *nullable add, pfx##_t *nullable del,                      \
          pfx##_t *nullable inter, pfx##_cmp_b nonnull cmp                   \
      )                                                                      \
      {                                                                      \
          __qvector_diff(                                                    \
              &vec1->qv, &vec2->qv, add ? &add->qv : NULL,                   \
              del ? &del->qv : NULL, inter ? &inter->qv : NULL,              \
              sizeof(val_t), alignof(val_t), (qvector_cmp_b)cmp              \
          );                                                                 \
      }                                                                      \
      __attr_unused__ static inline void pfx##_uniq(                         \
          pfx##_t *nonnull vec, pfx##_cmp_b nonnull cmp,                     \
          pfx##_del_b nullable del                                           \
      )                                                                      \
      {                                                                      \
          __qvector_uniq(                                                    \
              &vec->qv, sizeof(val_t), (qvector_cmp_b)cmp,                   \
              (qvector_del_b)del                                             \
          );                                                                 \
      }                                                                      \
      __attr_unused__ static inline int pfx##_bisect(                        \
          const pfx##_t *nonnull vec, cval_t v, bool *nullable found,        \
          pfx##_cmp_b nonnull cmp                                            \
      )                                                                      \
      {                                                                      \
          return __qvector_bisect(                                           \
              &vec->qv, sizeof(val_t), &v, found, (qvector_cmp_b)cmp         \
          );                                                                 \
      }                                                                      \
      __attr_unused__ static inline int pfx##_find(                          \
          const pfx##_t *nonnull vec, cval_t v, bool sorted,                 \
          pfx##_cmp_b nonnull cmp                                            \
      )                                                                      \
      {                                                                      \
          return __qvector_find(                                             \
              &vec->qv, sizeof(val_t), &v, sorted, (qvector_cmp_b)cmp        \
          );                                                                 \
      }                                                                      \
      __attr_unused__ static inline bool pfx##_contains(                     \
          const pfx##_t *nonnull vec, cval_t v, bool sorted,                 \
          pfx##_cmp_b nonnull cmp                                            \
      )                                                                      \
      {                                                                      \
          return __qvector_contains(                                         \
              &vec->qv, sizeof(val_t), &v, sorted, (qvector_cmp_b)cmp        \
          );                                                                 \
      }                                                                      \
      __attr_unused__ static inline void pfx##_deep_extend(                  \
          pfx##_t *nonnull vec_dst, pfx##_t *nonnull vec_src,                \
          pfx##_cpy_b nonnull cpy_f                                          \
      )                                                                      \
      {                                                                      \
          __qvector_deep_extend(                                             \
              &vec_dst->qv, &vec_src->qv, sizeof(val_t), alignof(val_t),     \
              (qvector_cpy_b)cpy_f                                           \
          );                                                                 \
      }
#else
...
#define qv_init_static(vec, _tab, _len)                                      \
    ({                                                                       \
        ...
        __qvector_init(                                                      \
            &__vec->qv, (void *)__tab, __len, __len, &mem_pool_static        \
        );                                                                   \
        __vec;                                                               \
    })
...
#define qv_inita(vec, size)                                                  \
    ({                                                                       \
        ...
        typeof(*(vec)) *__vec = (vec);                                       \
        __qvector_init(                                                      \
            &__vec->qv, alloca(_sz), 0, __size, &mem_pool_static             \
        );                                                                   \
        __vec;                                                               \
    })
#define mp_qv_init(mp, vec, size)                                            \
    ({                                                                       \
        ...
        typeof(*(vec)) *__vec = (vec);                                       \
        __qvector_init(                                                      \
            &__vec->qv, mp_new_raw(_mp, __qv_typeof(vec), __size), 0,        \
            __size, _mp                                                      \
        );                                                                   \
        __vec;                                                               \
    })
...
#define qv_qsort(vec, cmp)                                                   \
    ({                                                                       \
        ...
        int (*__cb)(__qv_typeof(vec) const *, __qv_typeof(vec) const *) =    \
            (cmp);                                                           \
        qsort(                                                               \
            __vec->qv.tab, __vec->qv.len, __qv_sz(vec),                      \
...
#define __qv_splice(vec, pos, rm_len, inserted_len)                          \
    ({                                                                       \
        (__qv_typeof(vec) *)__qvector_splice(                                \
            &(vec)->qv, __qv_sz(vec), __qv_align(vec), (pos), (rm_len),      \
            (inserted_len)                                                   \
        );                                                                   \
    })
...
#define qv_splice(vec, pos, rm_len, inserted_values, inserted_len)           \
    ({                                                                       \
        ...
        (__qv_typeof(vec) *)qvector_splice(                                  \
            &__vec->qv, __qv_sz(vec), __qv_align(vec), (pos), (rm_len),      \
            (__vals), (inserted_len)                                         \
        );                                                                   \
    })
...
#define qv_grow(vec, extra)                                                  \
    ({                                                                       \
        (__qv_typeof(vec) *)qvector_grow(                                    \
            &(vec)->qv, __qv_sz(vec), __qv_align(vec), (extra)               \
        );                                                                   \
    })
#define qv_growlen(vec, extra)                                               \
    ({                                                                       \
        (__qv_typeof(vec) *)qvector_growlen(                                 \
            &(vec)->qv, __qv_sz(vec), __qv_align(vec), (extra)               \
        );                                                                   \
    })
...
#define qv_grow_get_alloc_size(vec, extra)                                   \
    ({                                                                       \
        __qvector_grow_get_new_alloc_size(                                   \
            &(vec)->qv, __qv_sz(vec), (extra)                                \
        );                                                                   \
    })
...
#define qv_copy(vec_out, vec_in)                                             \
    ({                                                                       \
        ...
        __vec_out->len = 0;                                                  \
        p_copy(                                                              \
            qv_growlen(__vec_out, __vec_in->len), __vec_in->tab,             \
            __vec_in->len                                                    \
        );                                                                   \
        __vec_out;                                                           \
    })
