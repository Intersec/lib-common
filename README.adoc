...
(Tag-Length-Value) packers/unpackers. The IOP concept is similar to the
Google Protocol Buffers.

The IOP library is used to transmit data over the network in a safe manner. It
deals with data integrity checking, backward-compatibility issues, etc. It is
also used to exchange data between different languages or to provide a generic
...
waf:: build system
uv:: managing Python dependencies
...
Most dependencies can be installed using your own Linux distribution
...
`waf` versions 2.1.9 is known to work fine. Running the latest 2.1.x `waf`
...
`uv` is used to install Python dependencies used during compilation (Cython)
and in tests.
...
which just consists of running:
----------------------------------
$ waf
or
$ waf build
...
* `waf check`: run the tests in the current directory (defined in the `ZFile`)
               and in its sub-directories.
               The following variants also exist (cf `waf --help` for the
               details): `fast-check`, `www-check`, `selenium`,
               `fast-selenium`.
* `waf check-retry`: based on the content of the environment variable named
                     `Z_LIST_RETRY` (items separated by a comma), run a
                     dedicated list of tests (zchk, python or .feature files).
                     The following variant also exists (cf `waf --help` for
                     the details): `fast-selenium-retry`.
* `waf tags`: generate tags using ctags.
* `waf etags`: generate tags for emacs using ctags.
* `waf ruff`: run ruff checks on all python files.
* `waf ruff-fix`: run ruff check with automatic fix option on all python files
* `waf pyrefly`: run pyrefly checks on committed python files.
...
`NOCHECK` (boolean)::
    ...
    You may want to set it to speed up the build.
...
`UV_EXTRA` (string)::
    Specify additional optional dependencies that should be installed by uv.
    The value of this variable is a list of extra names (separated by spaces
    or commas), which will be installed alongside the release and dev
    dependencies.
...
The `static-checks.py` script runs linters on the codebase. It performs the
following checks:

* `ruff check`: lint Python files for code quality issues.
* `ruff format --check`: verify that Python files are properly formatted.
* `pyrefly`: run type checking on Python files.

To run the checks on the entire codebase:
---------------------------------
$ ./static-checks.py
---------------------------------

To run only on staged files (used by the git pre-commit hook):
---------------------------------
$ ./static-checks.py --staged-files
---------------------------------
...
Copyright (C) 2005-2026 by Intersec SA and the individual contributors to lib-common.
