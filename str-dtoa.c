...
/* This "shadow double" type avoid ugly shadowing used in previous version */
typedef union { double d; ULong w[2]; } shdouble;
#ifdef IEEE_8087
#define word0(shx) ((shx).w[1])
#define word1(shx) ((shx).w[0])
#else
#define word0(shx) ((shx).w[0])
#define word1(shx) ((shx).w[1])
#endif
...
#endif
{
    ...
    shx.d = _x_;
    L = (word0(shx) & Exp_mask) - (P-1)*Exp_msk1;
...
d2b
#ifdef KR_headers
(_d_, e, bits) double _d_; int *e, *bits;
#else
(double _d_, int *e, int *bits)
#endif
{
    Bigint *b;
    int de, i, k;
...
dig_done:
    ...
    if (nd <= DBL_DIG
#ifndef RND_PRODQUOT
#if !defined(FLT_ROUNDS_IS_CONSTANT) || FLT_ROUNDS != 1
    && FLT_ROUNDS == 1
#endif
#endif
       ) {
...
#else
#if !defined(FLT_ROUNDS_IS_CONSTANT) || FLT_ROUNDS == 0
            if (FLT_ROUNDS == 0)
                aadj1.d += 0.5;
#endif
#endif
...
static char *
#ifdef KR_headers
nrv_alloc(s, rve, n) char *s, **rve; unsigned int n;
#else
nrv_alloc(CONST char *s, char **rve, unsigned int n)
#endif
{
    char *rv, *t;
...
dtoa
#ifdef KR_headers
(_d_, mode, ndigits, decpt, sign, rve)
double _d_; int mode, ndigits, *decpt, *sign; char **rve;
#else
(double _d_, int mode, int ndigits, int *decpt, int *sign, char **rve)
#endif
{
...
#define j1 jj1  // hide Bessel function from <math.h>
    int j, j1, k, k_check, leftright, m2, m5, s2, s5;
    ...
    if (word0(shd) & Sign_bit) {
        /* set sign for everything, including 0's and NaNs */
        *sign = 1;
        word0(shd) &= ~Sign_bit;        /* clear sign bit */
    }
    else
        *sign = 0;
...
#ifdef IEEE_Arith
    if ((word0(shd) & Exp_mask) == Exp_mask)
#else
        if (word0(shd)  == 0x8000)
#endif
        {
            ...
            *decpt = 9999;
#ifdef IEEE_Arith
            if (!word1(shd) && !(word0(shd) & 0xfffff))
    ...
    b = d2b(shd.d, &be, &bbits);
#ifdef Sudden_Underflow
    i = (int)((word0(shd) >> Exp_shift1) & (Exp_mask >> Exp_shift1));
#else
    if ((i = (int)((word0(shd) >> Exp_shift1) & (Exp_mask >> Exp_shift1))) != 0)
    {
#endif
        d2.d = shd.d;
        word0(d2) &= Frac_mask1;
        ...
        x = i > 32  ? word0(shd) << (64 - i) | word1(shd) >> (i - 32)
                    : word1(shd) << (32 - i);
        d2.d = x;
...
#ifndef MODE_0_ONLY     /* because ilim = -1 */
            if (i == ilim) {
            //round9:
                shd.d += shd.d;
                if (shd.d > ds || (shd.d == ds && (L & 1))) {
#ifndef MODE_0_ONLY
                bump_up:
#endif
                    while (*--s == '9') {
    ...
    spec_case = 0;
    if (mode < 2) {
        if (!word1(shd) && !(word0(shd) & Bndry_mask)
#ifndef Sudden_Underflow
        && (word0(shd) & Exp_mask)
#endif
           ) {
...
#if 0
int ecvt_r(double d, int ndigits, int *decpt, int *sign,
           char *buf, int len)
{
    char *p, *p1, *end, *q, *q_end;
