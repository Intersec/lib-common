...
[source,c]
----
typedef struct stupid_local_type_t {
    bool has_caught;
    sb_t buf;
} stupid_local_type_t;
...
[source,c]
----
typedef struct apply_to_map_ctx_t {
    thr_job_t job;
    mapt_t *map;
} apply_to_map_ctx_t;
...
{
    apply_to_map_ctx_t *ctx = container_of(job, apply_to_map_ctx_t, job);
...
void apply_to_all_map(qv_t(map) *maps)
{
    ...
    thr_syn_init(&syn);
    qv_for_each_pos(map, pos, maps) {
        apply_to_map_ctx_t *ctx = p_new(apply_to_map_ctx_t, 1);
