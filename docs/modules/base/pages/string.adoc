...
void foo_wipe(foo_t *foo)
{
   lstr_wipe(&bar->name);
}
GENERIC_DELETE(foo_t, foo)

typedef struct bar_t {
   lstr_t name;
} bar_t;

void bar_wipe(bar_t *bar)
{
   lstr_wipe(&bar->name);
}
GENERIC_DELETE(bar_t, bar)
 ...
 */
foo_t *bar_to_foo(bar_t **bar)
{
   foo_t *foo = p_new(foo_t, 1);
 ...
 */
foo_t *foo_from_bar(const bar_t *bar)
{
   foo_t *foo = p_new(foo_t, 1);
 ...
 */
foo_t *copy_foo_from_bar(const bar_t *bar)
{
   foo_t *foo = p_new(foo_t, 1);
 ...
 */
int parse_record(pstream_t *ps, user_t *user)
{
 ...
 * are not ignored.
 */
int parse_record2(pstream_t *ps, user_t *user)
{
    pstream_t n;
