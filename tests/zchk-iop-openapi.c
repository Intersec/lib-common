...
static int t_z_load_openapi_file(const char *filename, lstr_t *file)
{
    const char *path;
...
static int z_check_yaml(
    iop_openapi_t *openapi, const char *filename, bool remove_schemas
)
{
    t_scope;
    yaml_data_t data;
...
Z_GROUP_EXPORT(iop_openapi)
{
    ...
    Z_TEST(doc, "test the whole doc generation") {
        ...
        oa = t_new_iop_openapi(
            iop_env_ctx, LSTR("zoomin"), LSTR("0.2.3"), NULL, LSTR("tes")
        );
        t_iop_openapi_set_description(oa, LSTR("sheo"));
        t_iop_openapi_set_security(
            oa, LSTR("my_sec"), OPENAPI_SECURITY_BASIC_HTTP
        );
        t_iop_openapi_set_server(
    ...
    Z_TEST(iop_mod, "test paths generation of IOP modules") {
        ...
        oa = t_new_iop_openapi(
            iop_env_ctx, LSTR("yay"), LSTR("0.0.1"), tstiop__t__modp,
            LSTR("route")
        );
        Z_HELPER_RUN(z_check_yaml(oa, "iface_t.yml", false));

        oa = t_new_iop_openapi(
            iop_env_ctx, LSTR("yay"), LSTR("0.0.1"), tstiop__my_mod_a__modp,
            LSTR("yay")
        );
        ...
        oa = t_new_iop_openapi(
            iop_env_ctx, LSTR("yay"), LSTR("0.0.1"), tstiop__both_iface__modp,
            LSTR("route")
        );
    ...
    Z_TEST(dox, "test inclusion of comments documentation") {
        ...
        oa = t_new_iop_openapi(
            iop_env_ctx, LSTR("tstdox"), LSTR("1.0.1"),
