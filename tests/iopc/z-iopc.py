...
class IopcTest(z.TestCase):
    ...
    def run_iopc_pass(
        self, iop: str, lang: str = '', class_id_range: str = ''
    ) -> None:
    ...
    def check_file(
        self, file_name: str, string_list: list[str], wanted: bool = True
    ) -> None:
    ...
    def check_code_gen_lang(
        self, pkg_name: str, pkg_path: str, lang: str
    ) -> None:
    ...
    def test_ambiguous_enum(self) -> None:
        ...
        f = 'enum5.iop'
        self.run_iopc(
            f, True, 'enum field identifier `MY_ENUM_A` is ambiguous'
        )
        self.run_gcc(f)
    ...
    def test_invalid_comma_enum(self) -> None:
        self.run_iopc(
            'enum_invalid_comma.iop', False, '`,` expected on every line'
        )
    ...
    def test_snmp_invalid_missing_index(self) -> None:
        self.run_iopc(
            'snmp_invalid_missing_index.iop',
            False,
    ...
    def test_integer_ext_invalid(self) -> None:
        self.run_iopc(
            'integer_ext_invalid.iop', False, 'invalid integer extension'
        )

    def test_unterminated_string(self) -> None:
        self.run_iopc(
            'string_val_unterminated.iop', False, '4:27:unterminated string'
        )

    def test_unterminated_cstring(self) -> None:
        self.run_iopc(
            'cstring_val_unterminated.iop', False, '4:28:unterminated cstring'
        )

    def test_unterminated_comment(self) -> None:
        self.run_iopc(
            'unterminated_comment.iop', False, '6:0:unterminated comment'
        )
    ...
    def test_defval(self) -> None:
        tests_invalid = [
            ...
            {'f': 'defval_int_max.iop', 's': 'violation of @max constraint'},
            {'f': 'defval_int_min.iop', 's': 'violation of @min constraint'},
            {
    ...
    def test_inheritance_invalid_multiple_inheritance(self) -> None:
        self.run_iopc(
            'inheritance_invalid_multiple_inheritance.iop',
            False,
    ...
    def test_inheritance_invalid_types(self) -> None:
        self.run_iopc(
            'inheritance_invalid_types1.iop',
            False,
            '`{` expected, but got `:`',
        )
        self.run_iopc(
            'inheritance_invalid_types2.iop',
            False,
            '`{` expected, but got `:`',
        )
        self.run_iopc(
            'inheritance_invalid_types3.iop',
            False,
            'parent object `Father` is not a class',
        )
        self.run_iopc_fail(
            'inheritance_invalid_types4.iop', 'only classes can be abstrac'
        )

    def test_inheritance_invalid_circular1(self) -> None:
        self.run_iopc(
            'inheritance_invalid_circular1.iop',
            False,
            [
                'circular dependency',
    ...
    def test_inheritance_invalid_circular2(self) -> None:
        self.run_iopc(
            'inheritance_invalid_circular2_pkg_a.iop',
            False,
            'circular dependency',
        )
        self.run_iopc(
            'inheritance_invalid_circular2_pkg_b.iop',
            False,
            'circular dependency',
        )
    ...
    def test_inheritance_invalid_duplicated_fields(self) -> None:
        self.run_iopc(
            'inheritance_invalid_duplicated_fields.iop',
            False,
    ...
    def test_inheritance_invalid_ids(self) -> None:
        self.run_iopc(
            'inheritance_invalid_id_small.iop',
            False,
            'id is too small (must be >= 0, got -1)',
        )
        self.run_iopc(
            'inheritance_invalid_id_large.iop',
            False,
            'id is too large (must be <= 65535, got 65536)',
        )
        self.run_iopc(
            'inheritance_invalid_id_missing.iop',
            False,
            'integer expected, but got identifier instead',
        )
        self.run_iopc(
            'inheritance_invalid_id_duplicated.iop',
            False,
    ...
    def test_inheritance_invalid_static(self) -> None:
        self.run_iopc(
            'inheritance_invalid_static_struct.iop',
            False,
            'static keyword is only authorized for class fields',
        )
        self.run_iopc(
            'inheritance_invalid_static_no_default.iop',
            False,
            'static fields of non-abstract classes must have a default value',
        )
        self.run_iopc(
            'inheritance_invalid_static_repeated.iop',
            False,
            'repeated static members are forbidden',
        )
        self.run_iopc(
            'inheritance_invalid_static_optional.iop',
            False,
            'optional static members are forbidden',
        )
        self.run_iopc(
            'inheritance_invalid_static_type_mismatch.iop',
            False,
            'incompatible type for static field `toto`: should be `int`',
        )
        self.run_iopc(
            'inheritance_invalid_static_already_defined.iop',
            False,
            'field `toto` already defined by parent `A`',
        )
        self.run_iopc(
            'inheritance_invalid_static_abstract_not_defined.iop',
            False,
            'error: class `D1` must define a static '
    ...
    def test_inheritance_class_id_ranges(self) -> None:
        ...
        self.run_iopc_fail('', 'invalid class-id-range', class_id_range='-10')
        self.run_iopc_fail(
            '', 'invalid class-id-range', class_id_range='-10-10'
        )
        self.run_iopc_fail(
            '', 'invalid class-id-range', class_id_range='65536-10'
        )
        self.run_iopc_fail(
            '', 'invalid class-id-range', class_id_range='10-9'
        )
        self.run_iopc_fail(
            '', 'invalid class-id-range', class_id_range='10-65536'
        )

        self.run_iopc_pass('inheritance_pkg_b.iop', class_id_range='')
        self.run_iopc_pass('inheritance_pkg_b.iop', class_id_range='10-19')
        self.run_iopc(
            'inheritance_pkg_b.iop',
            False,
    ...
    def test_inheritance_invalid_local_pkg(self) -> None:
        self.run_iopc(
            'inheritance_invalid_local_pkg.iop',
            False,
    ...
    def test_inheritance_tag_static_invalid(self) -> None:
        self.run_iopc(
            'inheritance_tag_static_invalid.iop',
            False,
    ...
    def test_typedef_invalid(self) -> None:
        ...
        # compilation
        self.run_iopc(
            'typedef_invalid_13.iop',
            False,
            'attribute minOccurs does not apply to required typedefs',
        )
        self.run_iopc(
            'typedef_invalid_14.iop',
            False,
            'cannot declare repeated optional fields',
        )
        self.run_iopc(
            'typedef_invalid_15.iop',
            False,
            'cannot declare repeated optional fields',
        )
        self.run_iopc(
            'invalid_union_empty.iop',
            False,
    ...
    def test_void_types(self) -> None:
        ...
        self.run_iopc(
            'invalid_void_repeated.iop',
            False,
            'repeated void types are forbidden',
        )
        self.run_iopc(
            'invalid_void_default.iop',
            False,
        ...
        self.run_iopc_pass('void_private_req_in_struct.iop', lang='C,json')
    ...
    def test_attrs_invalid_1(self) -> None:
        self.run_iopc(
            'attrs_invalid_1.iop', False, 'incorrect attribute name'
        )
    ...
    def test_attrs_invalid_8(self) -> None:
        self.run_iopc(
            'attrs_invalid_8.iop', False, 'unknown field c in MyUnion'
        )

    def test_attrs_invalid_9(self) -> None:
        self.run_iopc(
            'attrs_invalid_9.iop', False, 'unknown field c in MyUnion'
        )

    def test_attrs_invalid_10(self) -> None:
        self.run_iopc(
            'attrs_invalid_10.iop',
            False,
    ...
    def test_attrs_invalid_11(self) -> None:
        self.run_iopc(
            'attrs_invalid_11.iop', False, 'unknown field c in MyUnion'
        )

    def test_attrs_invalid_12(self) -> None:
        self.run_iopc(
            'attrs_invalid_12.iop', False, 'unknown field c in MyUnion'
        )

    def test_attrs_invalid_13(self) -> None:
        self.run_iopc(
            'attrs_invalid_13.iop', False, 'unknown field C in MyEnum'
        )

    def test_attrs_invalid_14(self) -> None:
        self.run_iopc(
            'attrs_invalid_14.iop', False, 'unknown field C in MyEnum'
        )

    def test_attrs_invalid_15(self) -> None:
        self.run_iopc(
            'attrs_invalid_15.iop',
            False,
    ...
    def test_attrs_invalid_17(self) -> None:
        ...
        self.run_iopc(
            f, False, 'all static attributes must be declared first'
        )

    def test_attrs_invalid_18(self) -> None:
        f = 'attrs_invalid_18.iop'
        self.run_iopc(
            f, False, 'error: invalid ctype `user_t`: missing __t suffix'
        )

    def test_attrs_invalid_enumval(self) -> None:
        self.run_iopc(
            'attrs_invalid_enumval.iop',
            False,
    ...
    def test_attrs_invalid_private_struct(self) -> None:
        ...
        self.run_iopc(f, False, 'attribute private does not apply to struct')

    def test_attrs_invalid_private_union(self) -> None:
        f = 'attrs_invalid_private_union.iop'
        self.run_iopc(f, False, 'attribute private does not apply to union')

    def test_attrs_invalid_private_enum(self) -> None:
        f = 'attrs_invalid_private_enum.iop'
        self.run_iopc(f, False, 'attribute private does not apply to enum')
    ...
    def test_attrs_empty_ctype(self) -> None:
        ...
        self.run_iopc(
            f, False, 'attribute ctype expects at least one argument'
        )

    def test_attrs_bad_ctypes(self) -> None:
        f = 'attrs_bad_ctypes.iop'
        self.run_iopc(f, False, 'invalid ctype `invalid`: missing __t suffix')

    def test_attrs_bad_nb_args(self) -> None:
        f = 'attrs_bad_nb_args.iop'
        self.run_iopc(f, False, 'attribute prefix expects 1 arguments, got 0')
    ...
    def test_generic_json(self) -> None:
        ...
        for lang in ['json', 'c']:
            self.check_ref(g, lang)
        self.run_iopc(
            'json_generic_invalid1.iop',
            False,
            'error: `:` expected, but got `)` instead',
        )
        self.run_iopc(
            'json_generic_invalid2.iop',
            False,
            'error: invalid token when parsing json value',
        )
        self.run_iopc(
            'json_generic_invalid3.iop',
            False,
            'error: string expected, but got `,` instead',
        )
        self.run_iopc(
            'json_generic_invalid4.iop',
            False,
            'error: `]` expected, but got integer instead',
        )
        self.run_iopc(
            'json_generic_invalid5.iop',
            False,
    ...
    def test_generic_json_unterminated(self) -> None:
        self.run_iopc(
            'json_generic_invalid6.iop', False, '4:70:unterminated string'
        )

    def test_generic_invalid_name(self) -> None:
        self.run_iopc(
            'generic_attrs_invalid_name.iop',
            False,
    ...
    def test_reference_invalid(self) -> None:
        self.run_iopc(
            'reference_invalid_1.iop',
            False,
            'references can only be applied to structures or unions',
        )
        self.run_iopc(
            'reference_invalid_2.iop',
            False,
            'references can only be applied to structures or unions',
        )
        self.run_iopc(
            'reference_invalid_3.iop',
            False,
            'references can only be applied to structures or unions',
        )
        self.run_iopc(
            'reference_invalid_4.iop',
            False,
            'references can only be applied to structures or unions',
        )
        self.run_iopc('reference_invalid_5.iop', False, 'circular dependency')
        self.run_iopc(
            'reference_invalid_6.iop',
            False,
            'identifier expected, but got `?` instead',
        )
        self.run_iopc(
            'reference_invalid_7.iop',
            False,
            'identifier expected, but got `&` instead',
        )
        self.run_iopc(
            'reference_invalid_8.iop',
            False,
            'referenced static members are forbidden',
        )
        self.run_iopc('reference_invalid_9.iop', False, 'circular dependency')
    ...
    def test_same_as(self) -> None:
        ...
        self.check_file(
            ...
            wanted=True,
            string_list=[
                'same_as__struct1__desc_fields',
                'same as same_as.Struct1',
                'same_as__union1__desc_fields',
                'same as same_as.Union1',
                'same_as__class1__desc_fields',
                'same as same_as.Class1',
    ...
    def test_unknown_pkg(self) -> None:
        self.run_iopc_fail(
            'unknown_pkg.iop',
            'unable to find file `unknown.iop` in the include path',
        )

    def test_invalid_pkg_syntax(self) -> None:
        self.run_iopc_fail(
            'invalid_pkg_syntax.iop',
    ...
    def test_no_out_nor_throw(self) -> None:
        self.run_iopc_fail(
            'async_candidate.iop',
            ('no `out` nor `throw` for function `asyncCandidate`'),
        )

    def test_dup_struct(self) -> None:
        self.run_iopc(
            'duplicated_struct.iop',
            False,
    ...
    def test_dox_invalid(self) -> None:
        self.run_iopc_fail(
            'tstdox_invalid1.iop',
            ('doxygen unrelated `in` argument `unknown` for RPC `funA`'),
        )
        self.run_iopc_fail(
            'tstdox_invalid2.iop',
            ('error: invalid identifier when parsing json value'),
        )
    ...
    def test_check_name(self) -> None:
        ...
        self.run_iopc_fail(
            'check_name_field.iop',
            "error: identifier 'You_Shall_Not_Pass' contains a _",
        )
        self.run_iopc_fail(
            'check_name_rpc.iop', 'error: you_Shall_Not_Pass contains a _'
        )

    def test_missing_iface(self) -> None:
        self.run_iopc_fail(
            'missing_iface.iop',
            'error: unable to find any pkg providing interface `MyIfaceA`',
        )

    def test_missing_module(self) -> None:
        self.run_iopc_fail(
            'missing_module.iop',
            'error: unable to find any pkg providing module `MyModuleA`',
        )
