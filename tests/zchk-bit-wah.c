...
#define ADD_N_CHECK(nbits, exp_shift)                                        \
    ...
    ADD_N_CHECK(
        WAH_BIT_IN_WORD - 5 + WAH_BIT_IN_WORD * 2, 5 * WAH_BIT_IN_WORD
    );
...
Z_GROUP_EXPORT(wah)
{
    ...
    Z_TEST(set_bitmap) { /* {{{ */
        ...
        for (int i = 0; i < countof(data); i++) {
            for (int j = 0; j < 8; j++) {
                Z_ASSERT_EQ(
                    !!(data[i] & (1 << j)), !!wah_get(&map, i * 8 + j),
                    "invalid byte %d, bit %d", i, j
                );
                Z_ASSERT_EQ(
                    !!(data[i] & (1 << j)), !!wah_get(&map2, i * 8 + j),
                    "invalid byte %d, bit %d", i, j
                );
            }
        }
    ...
    Z_TEST(for_each) { /* {{{ */
        ...
        wah_for_each_0(en, &map)
        {
            ...
            Z_ASSERT_CMP(en.key, <, bitsizeof(data), "enumerate too far");
            Z_ASSERT(
                !(data[en.key >> 3] & (1 << (en.key & 0x7))), "bit %d is set",
                (int)en.key
            );
        }
    ...
    Z_TEST(binop) { /* {{{ */
        ...
        const byte data2[] = {
            ...
            0x20, 0x00, /* 76, 85                              (96) */
            0x00, 0x00,
            0xc0, 0x20, /* 118, 119, 125                       (128)*/
            0xff, 0xfc,
            0xff, 0x12 /* 128 -> 135, 138 -> 151, 153, 156    (160)*/
        };

        /* And result:
         *                                                                 (32)
         * 63 (64) 76, 85 (96) 118, 119 (128) 140, 150 (160)
         */

        /* Or result:
         * 0 -> 4, 26, 27, 31 (32) 32 -> 63 (64) 64 -> 95 (96) 96 -> 119, 125,
         * 127                                             (128) 128 -> 135,
         * 138 -> 151, 153, 156                                (160) (192)
         *                                                                 (224)
         *                                                                 (256)
         * 280, 285 (288)
         */
        ...
        /* And-Not result
         * 0, 1, 2, 3, 4, 26, 27, 31 (32) 32 -> 62 (64) 64 -> 75, 77 -> 84, 86
         * -> 95                                    (96) (128) (160) (192)
         *                                                                 (224)
         *                                                                 (256)
         * 280, 285 (288)
         ...
         *                                                                 (64)
         *                                                                 (96)
         * 125 (128) 128 -> 135, 138, 139, 141 -> 149, 151, 153, 156 (160)
         */
    ...
    Z_TEST(redmine_4576) { /* {{{ */
        wah_t map;
        const byte data[] = {0x1f, 0x00, 0x1f, 0x1f, 0x00, 0x00, 0x00, 0x00,
    ...
    Z_TEST(non_reg_and) { /* {{{ */
        t_scope;
        uint32_t src_data[] = {
            0x00000519, 0x00000000, 0x80000101, 0x00000000
        };
        uint32_t other_data[] = {0x00000000, 0x00000002, 0x80000010,
                                 0x00000003, 0x0000001d, 0x00000001,
                                 0x00007e00, 0x0000001e, 0x00000000};
        wah_t src;
        wah_t other;
        wah_t res;
    ...
    Z_TEST(skip1s) { /* {{{ */
        ...
        wah_for_each_1(en, &map)
        {
            for (uint64_t i = pos; i < bc; i++) {
                ...
                Z_ASSERT_EQ(
                    en_skip.word_en.state, en_incr.word_en.state,
    ...
    Z_TEST(buckets) { /* {{{ */
        ...
        uint32_t literal[] = {
            ...
            0x12345678, 0x12345678, 0x00000001,
        };
...
#define CHECK_WAH(_nb_buckets, _len)                                         \
    do {                                                                     \
        ...
        Z_ASSERT_EQ(                                                         \
            map1.active,                                                     \
            5 * WAH_BIT_IN_WORD + membitcount(literal, countof(literal) * 4) \
        );                                                                   \
                                                                             \
        for (uint64_t i = 0; i < 3 * 5 * WAH_BIT_IN_WORD; i++) {             \
            if (i >= 5 * WAH_BIT_IN_WORD && i < 2 * 5 * WAH_BIT_IN_WORD) {   \
        ...
        for (uint64_t i = 0; i < 5 * WAH_BIT_IN_WORD + 2; i++) {             \
            Z_ASSERT_EQ(                                                     \
                wah_get(&map1, i + 15 * WAH_BIT_IN_WORD),                    \
                !!TST_BIT(literal, i)                                        \
            );                                                               \
        }                                                                    \
    } while (0)
...
#undef CHECK_WAH
    ...
    Z_TEST(t_wah_get_storage_lstr) { /* {{{ */
        ...
        {
            Z_ASSERT_EQ(
                en.key, bits_pos[pos], "bad bit position for bit [%d]", pos
            );
            pos++;
        }
        Z_ASSERT_EQ(
            pos, countof(bits_pos), "missing bits in the WAH gotten from data"
        );
...
#define TEST_PAD32(nbits, res_len)                                           \
    do {                                                                     \
        ...
        wah_pad32(&map);                                                     \
        Z_ASSERT_EQ(                                                         \
            map.len, res_len, "invalid padding with %ju bits",               \
            (uint64_t)nbits                                                  \
        );                                                                   \
    } while (0)
...
#undef TEST_PAD32
    ...
    Z_TEST(fuzzing_nr_1) { /* {{{ */
        ...
        static const uint8_t data[32] = {
            ...
            0x00, 0x00, 0x04, 0x00, 0x04, 0x00, 0x04, 0x00, 0x08, 0x00, 0x00,
            0x00, 0x00, 0x01, 0x00, 0x00, 0x01, 0x00, 0x21, 0x00, 0x00,
        };
