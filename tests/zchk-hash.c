...
Z_GROUP_EXPORT(hash)
{
    ...
    Z_TEST(murmur_hash3_x86_32) {
        lstr_t s = LSTR(
            "Est-ce que vous voulez etre ma femme ? "
...
Z_GROUP_EXPORT(sha2)
{
    ...
    Z_TEST(hmac) {
        ...
        for (int i = 0; i < 14; i++) {
            ...
            if (j == 5 || j == 6) {
                ...
                sha2_hmac_starts(
                    &ctx, sha2_hmac_test_key[j], sha2_hmac_test_keylen[j], k
                );
            }
            sha2_hmac_update(
                &ctx, sha2_hmac_test_buf[j], sha2_hmac_test_buflen[j]
            );
    ...
    Z_TEST(crypt) {
        ...
        static const struct {
            ...
            {"This is just a test", 5000, "toolongsaltstring", 5000,
             "toolongsaltstrin",
             "Un/5jzAHMgOGZ5.mWJpuVolil07guHPvOW8mGRcvxa5"},
            {"a very much longer text to encrypt.  This one even stretche"
             "s over morethan one line.",
             1400, "anotherlongsaltstring", 1400, "anotherlongsalts",
            ...
            {"a short string", 123456, "asaltof16chars..", 123456,
             "asaltof16chars..",
             "gP3VQ/6X7UUEW3HkBn2w1/Ptq2jxPyzV/cZKmF/wJvD"},
            {"the minimum number is still observed", 10, "roundstoolow", 1000,
             "roundstoolow", "yfvwcWrQ8l/K0DAWyuPMDNHpIVlTQebY9l/gL972bIC"},
        };
        ...
        for (int i = 0; i < countof(tests_crypt); ++i) {
            ...
            sha2_crypt(
                tests_crypt[i].input, strlen(tests_crypt[i].input),
                tests_crypt[i].salt, strlen(tests_crypt[i].salt),
                tests_crypt[i].rounds, &crypt_buf
            );

            expected = t_lstr_fmt(
                "$5$rounds=%d$%s$%s", tests_crypt[i].expected_rounds,
                tests_crypt[i].expected_salt, tests_crypt[i].expected_hash
            );
            Z_ASSERT_LSTREQUAL(LSTR_SB_V(&crypt_buf), expected);

            Z_ASSERT_N(
                sha2_crypt_parse(LSTR_SB_V(&crypt_buf), &rounds, &salt, &hash)
            );
            Z_ASSERT_EQ(rounds, tests_crypt[i].expected_rounds);
            Z_ASSERT_LSTREQUAL(
                LSTR_PS_V(&salt), LSTR(tests_crypt[i].expected_salt)
            );
            Z_ASSERT_LSTREQUAL(
                LSTR_PS_V(&hash), LSTR(tests_crypt[i].expected_hash)
            );
        }
...
#define CHECK_FAIL(str)                                                      \
    Z_ASSERT_NEG(sha2_crypt_parse(LSTR(str), NULL, NULL, NULL))
        ...
        /* Wrong rounds format */
        CHECK_FAIL(
            "$5$rounds=a10000$salt"
            "$3xv.VbSHBb41AL9AvLeujZkZRBAwqFMz2.opqey6IcA"
        );
        /* Rounds count too low */
        CHECK_FAIL(
            "$5$rounds=999$salt$3xv.VbSHBb41AL9AvLeujZkZRBAwqFMz2.opqey6IcA"
        );
        /* Rounds count too high */
        CHECK_FAIL(
            "$5$rounds=1000000000$salt"
            "$3xv.VbSHBb41AL9AvLeujZkZRBAwqFMz2.opqey6IcA"
        );
        /* Wrong rounds format */
        CHECK_FAIL(
            "$5$rounds=10000a$salt"
            "$3xv.VbSHBb41AL9AvLeujZkZRBAwqFMz2.opqey6IcA"
        );
        ...
        /* Empty salt */
        CHECK_FAIL(
            "$5$rounds=10000$$3xv.VbSHBb41AL9AvLeujZkZRBAwqFMz2.opqey6IcA"
        );
        /* Salt string too long */
        CHECK_FAIL(
            "$5$rounds=10000$toolongsaltstring"
            "$3xv.VbSHBb41AL9AvLeujZkZRBAwqFMz2.opqey6IcA"
        );
...
static const int sha4_test_buflen[3] = {3, 112, 1000};
...
static const int sha4_hmac_test_keylen[7] = {20, 4, 20, 25, 20, 131, 131};
...
static const int sha4_hmac_test_buflen[7] = {8, 28, 50, 50, 20, 54, 152};
    ...
    Z_TEST(hash) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (j == 2) {
                ...
                sha4_update(&ctx, sha4_test_buf[j], sha4_test_buflen[j]);
            }
    ...
    Z_TEST(hmac) {
        ...
        for (int i = 0; i < 14; i++) {
            ...
            if (j == 5 || j == 6) {
                ...
                sha4_hmac_starts(
                    &ctx, sha4_hmac_test_key[j], sha4_hmac_test_keylen[j], k
                );
            }
            sha4_hmac_update(
                &ctx, sha4_hmac_test_buf[j], sha4_hmac_test_buflen[j]
            );
...
static const byte aes_test_cfb_dec[3][16] = {
    {0xBA, 0x75, 0x0C, 0xC9, 0x77, 0xF8, 0xD4, 0xE1, 0x3E, 0x0F, 0xB5, 0x46,
     0x2E, 0xA6, 0x33, 0xF6},
    {0xDB, 0x40, 0x4A, 0x98, 0x7B, 0xAA, 0xA3, 0xF3, 0x92, 0x35, 0xAD, 0x58,
     0x09, 0x9B, 0xFF, 0x6E},
    {0xA8, 0x17, 0x41, 0x0E, 0x76, 0x71, 0x60, 0xE5, 0xFD, 0x37, 0xC5, 0x43,
     0xCC, 0xC8, 0xD6, 0xDA}
};

static const byte aes_test_cfb_enc[3][16] = {
    {0x45, 0x62, 0xC5, 0xA1, 0xF9, 0x10, 0x8F, 0xE0, 0x87, 0x24, 0x25, 0x68,
     0xB5, 0x12, 0xF3, 0x8B},
    {0xB8, 0xD4, 0xD5, 0x09, 0xF5, 0xEE, 0x08, 0x38, 0x48, 0x9B, 0x9D, 0xAD,
     0x11, 0xB4, 0x2E, 0xD2},
    {0xE9, 0x10, 0x80, 0xDA, 0xEE, 0x2D, 0x81, 0xD9, 0x41, 0x78, 0x91, 0xD5,
     0x98, 0x78, 0xE1, 0xFA}
};
 ...
 */
static const byte des3_test_keys[24] = {0x01, 0x23, 0x45, 0x67, 0x89, 0xAB,
                                        0xCD, 0xEF, 0x23, 0x45, 0x67, 0x89,
                                        0xAB, 0xCD, 0xEF, 0x01, 0x45, 0x67,
...
static const byte des3_test_buf[8] = {0x4E, 0x6F, 0x77, 0x20,
                                      0x69, 0x73, 0x20, 0x74};
    ...
    Z_TEST(hmac) {
        ...
        for (int i = 0; i < 7; i++) {
            if (i == 5 || i == 6) {
                ...
                md5_hmac_starts(
                    &ctx, md5_hmac_test_key[i], md5_hmac_test_keylen[i]
                );
            }

            md5_hmac_update(
                &ctx, md5_hmac_test_buf[i], md5_hmac_test_buflen[i]
            );
