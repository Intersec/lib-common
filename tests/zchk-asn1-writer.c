...
enum test_tf { ASN1_TEST_TRUE = 666, ASN1_TEST_FALSE = 667 };
...
uint8_t il_test_input[] = {0x76, 0x80, 0x12, 0x02, 0x10, 0x00, 0x34,
                           0x01, 0x00, 0x55, 0x80, 0x04, 0x02, 0x12,
                           0x34, 0x78, 0x80, 0x80, 0x00, 0x00, 0x00,
                           0x00, 0x00, 0x00, 0x00};

uint8_t il_rec_input[] = {
    0x66, 0x80, 0x34, 0x80, 0x12, 0x01, 0x01, 0x12, 0x01,
    0x02, 0x00, 0x00, 0x34, 0x80, 0x12, 0x01, 0x03, 0x12,
    0x01, 0x04, 0x12, 0x01, 0x05, 0x00, 0x00, 0x00, 0x00,
};

uint8_t il_rec_uchoice_input[] = {
    0x23, 0x01, 0x01, 0xec, 0x80, 0xbb, 0x01, 0x01,
    0x16, 0x01, 0x42, 0x00, 0x00, 0x34, 0x01, 0x02,
};
...
static bool
simple_array_equal(const simple_array_t *a1, const simple_array_t *a2)
{
...
static bool
test_vector_equal(const test_vector_t *t1, const test_vector_t *t2)
{
    ...
    for (int i = 0; i < t1->choice.len; i++) {
        if (t1->choice.data[i].type != t2->choice.data[i].type) {
            e_trace(
                0, "FAIL (type %d != %d)", t1->choice.data[i].type,
                t2->choice.data[i].type
            );

            return false;
        }
        if (t1->choice.data[i].choice1 !=
            t2->choice.data[i].choice1) /* XXX */
        {
            e_trace(
                0, "FAIL (choice %d != %d)", t1->choice.data[i].choice1,
...
#define DEC(v, i)                                                            \
    ({                                                                       \
        ps = ps_init(v, sizeof(v));                                          \
        ber_decode_int32(&ps, i);                                            \
    })
...
#undef DEC
    ...
    Z_TEST(enc0, "asn1: BER encoder/decoder - constructed types") {
        static uint8_t const expected[] = {
            0xab, 0x01, 0xff, 0xcd, 0x05, 0x00, 0x87, 0x65, 0x43, 0x21,
            0xef, 0x01, 0x42, 0x64, 0x05, 0x00, 0x87, 0x65, 0x43, 0x21,
            0x64, 0x09, 0x00, 0x92, 0x34, 0x56, 0x78, 0x90, 0xab, 0xcd,
            0xef, 0x64, 0x08, 0x12, 0x34, 0x56, 0x78, 0x90, 0xab, 0xcd,
    ...
    Z_TEST(enc2, "asn1: BER encoder/decoder - constructed types") {
        static uint8_t const expected[] = {
            0x32, 0x30, 0xab, 0x01, 0xff, 0xcd, 0x05, 0x00, 0x87, 0x65, 0x43,
            0x21, 0xef, 0x01, 0x42, 0x64, 0x05, 0x00, 0x87, 0x65, 0x43, 0x21,
            0x64, 0x09, 0x00, 0x92, 0x34, 0x56, 0x78, 0x90, 0xab, 0xcd, 0xef,
            0x64, 0x08, 0x12, 0x34, 0x56, 0x78, 0x90, 0xab, 0xcd, 0xef, 0xbb,
            0x01, 0x01, 0x0f, 0x02, 0x02, 0x9a, 0x34, 0x0c, 0xab, 0x06, 0x73,
            0x74, 0x72, 0x69, 0x6e, 0x67, 0xb5, 0x02, 0x04, 0x0f,
        };
    ...
    Z_TEST(enc3, "asn1: BER encoder/decoder - constructed types") {
        static uint8_t const expected[] = {
            0x77, 0x0c, 0xab, 0x06, 0x73, 0x74, 0x72,
            0x69, 0x6e, 0x67, 0xb5, 0x02, 0x04, 0x0f,
        };
        test_3_t const t3 = {
    ...
    Z_TEST(reader, "asn1: BER reader test") {
        ...
        Z_ASSERT_EQUAL(
            rdr_out.str.s, rdr_out.str.len, exp_rdr_out.str.s,
            exp_rdr_out.str.len
        );
        Z_ASSERT_EQ(rdr_out.bstr.bit_len, exp_rdr_out.bstr.bit_len);
        Z_ASSERT_ZERO(memcmp(
            rdr_out.bstr.data, exp_rdr_out.bstr.data,
        ...
        Z_ASSERT_EQUAL(
            rdr_out.vec.vec.data, rdr_out.vec.vec.len,
    ...
    Z_TEST(array, "asn1: BER array (un)packing") {
        ...
        static simple_array_t const simple_array = {
            .array = {.data = rec1_array, .len = 4}
        };

        static uint8_t const exp_simple_array[] = {
            0xaa, 0x07, 0xbb, 0x01, 0x01, 0x16, 0x02, 0x01, 0x23,
            0xaa, 0x08, 0xbb, 0x01, 0x00, 0x16, 0x03, 0x04, 0x44,
            0x44, 0xaa, 0x06, 0xbb, 0x01, 0x00, 0x16, 0x01, 0x00,
            0xaa, 0x07, 0xbb, 0x01, 0x01, 0x16, 0x02, 0x00, 0x96
        };
        ...
        ps = ps_init(buf, len);
        Z_ASSERT_N(
            asn1_unpack(simple_array, &ps, t_pool(), &simple_array_out, false)
        );
    ...
    Z_TEST(choice, "asn1: BER choice (un)packing") {
        ...
        static uint8_t const exp_choice_no_skip[] = {0xec, 0x07, 0xbb,
                                                     0x01, 0x01, 0x16,
                                                     0x02, 0X34, 0x56};
        ...
        static test_u_choice_t const u_choice = {
            .i = 0x34, .choice = &in_u_choice
        };

        static uint8_t const exp_u_choice[] = {0x02, 0x01, 0x34,
                                               0x34, 0x01, 0x25};

        static uint8_t const choice_input[] = {0xec, 0x0d, 0xbb, 0x01, 0x01,
                                               0x55, 0x04, 0x00, 0x01, 0x02,
                                               0x03, 0x16, 0x2,  0x34, 0x56};
        ...
        Z_ASSERT_EQUAL(
            buf, len, exp_choice_no_skip, sizeof(exp_choice_no_skip)
        );

        Z_ASSERT_N(
            asn1_unpack(test_choice, &choice_ps, NULL, &choice, false)
        );
        ...
        ps = ps_init(buf, len);
        Z_ASSERT_N(
            asn1_unpack(test_u_choice, &ps, t_pool(), &u_choice_out, false)
        );
    ...
    Z_TEST(vector_array, "asn1: BER vectors/array") {
        ...
        static const uint8_t exp_test_vector[] = {0x34, 0x02, 0x01, 0x23,
                                                  0x23, 0x02, 0x04, 0x56,
                                                  0x45, 0x02, 0x07, 0x89};

        static const test_vector_t test_vector_in = {
            .choice = {.data = choice_vec, .len = 3}
        };

        static const test_array_t test_array_in = {
            .choice = {.data = choice_arr, .len = 3}
        };
        ...
        ps = ps_init(buf, len);
        Z_ASSERT_N(
            asn1_unpack(test_vector, &ps, t_pool(), &test_vector, false)
        );
        ...
        ps = ps_init(buf, len);
        Z_ASSERT_N(
            asn1_unpack(test_array, &ps, t_pool(), &test_array, false)
        );
    ...
    Z_TEST(asn1_skip_field, "asn1: asn1_skip_field()") {
        ...
        static const uint8_t fields[] = {
            0x01, 0x02, 0xab, 0xcd, 0xa1, 0x80, 0x01, 0x01, 0x02, 0x01,
            0x02, 0xfe, 0xdc, 0x00, 0x00, 0x04, 0x81, 0xa2, 0x01,
        };
        byte *long_field;
        int vlen;
        qv_t(i32) stack;
...
Z_GROUP_EXPORT(asn1_open_type)
{
    Z_TEST(open_type, "asn1: open type") {
        ...
        uint8_t want_ot[] = {0xa1, 0x03, 0x01, 0x02, 0x03, 0xa2,
                             0x05, 0x31, 0x32, 0x33, 0x34, 0x00};
...
static ASN1_CHOICE_DESC_BEGIN(nr_opt_ot_eoc_c, nr_opt_ot_eoc_type, type);
asn1_reg_sequence(
    nr_opt_ot_eoc_c, nr_opt_ot_eoc, a, ASN1_MK_TAG_C(CONTEXT_SPECIFIC, 1)
);
