...
//! logger_error!(logger, "an error occurred: {}", error_msg);
//! ```
...
use crate::bindings::{
    __logger_fatal, __logger_log, __logger_panic, logger_delete, logger_get_root, logger_has_level,
    logger_new, logger_t,
};
...
/// It can be created either as a new logger (owned) or from an existing
/// `logger_t` pointer (borrowed).
pub struct Logger<'a> {
    /// Pointer to the underlying C logger.
    inner: *mut logger_t,
    /// Whether this logger owns the underlying memory.
    owned: bool,
...
impl Logger<'_> {
    ...
    pub fn has_level(&self, level: i32) -> bool {
        unsafe { logger_has_level(self.inner, level) }
    }
    ...
    #[must_use]
    pub fn as_ptr(&self) -> *mut logger_t {
        self.inner
    }
...
impl Drop for Logger<'_> {
    fn drop(&mut self) {
        if self.owned {
            unsafe {
                logger_delete(&raw mut self.inner);
            }
        }
    }
}
